...
- 去掉 73 处 OCR 占位 caption「图示（原PDF第N页）」；`figures-inventory.tsv` 中 142 行 `alt` 内的多余制表符并入该列。
...
- README 标明当前 TeX/PDF 尚未严格校对，建议优先使用 `raw/` 原始扫描。
- 不再把空的 `reports/` 纳入版本库；审计 JSON 仍在本地生成。
