...
\centering
\fitfig{%
% fig-alt: 第9题 长方形 ABCD，E 在 AB 上、F 在 AD 上且 BE:BA=AF:AD，CE 与 BF 交于 P
\begin{tikzpicture}[x=0.85cm,y=0.85cm,line cap=round,line join=round,>=latex]
  ...
  \coordinate (E) at (0,{\t*\H});
  \coordinate (F) at ({\t*\W},{\H});
  \pgfmathsetmacro{\px}{\t*\t*\W/(1+\t*\t)}
  \pgfmathsetmacro{\py}{\t*\H/(1+\t*\t)}
  \coordinate (P) at ({\px},{\py});
  \draw[line width=0.8pt] (A) -- (B) -- (C) -- (D) -- cycle;
  \draw[line width=0.7pt] (B) -- (F);
  \draw[line width=0.7pt] (E) -- (C);
  \fill (A) circle (1.1pt);
  \fill (B) circle (1.1pt);
  \fill (C) circle (1.1pt);
  \fill (D) circle (1.1pt);
...
\centering
\fitfig{%
% fig-alt: 图6·20（1） 圆沿 x 轴滚动，定点 P 描旋轮线（摆线）；圆心 C、切点 A、垂足 M、水平辅助点 Q，∠PCQ=θ
\begin{tikzpicture}[x=0.55cm,y=0.55cm,line cap=round,line join=round,>=latex]
  \def\r{1.55}
  \def\th{1.05} % radians
  \pgfmathsetmacro{\cx}{\r*\th}
  \pgfmathsetmacro{\px}{\r*(\th-sin(\th r))}
  \pgfmathsetmacro{\py}{\r*(1-cos(\th r))}
  \coordinate (O) at (0,0);
  \coordinate (C) at ({\cx},\r);
  \coordinate (A) at ({\cx},0);
  \coordinate (P) at ({\px},{\py});
  \coordinate (M) at ({\px},0);
  \coordinate (Q) at ({\cx},{\py});
  \pgfmathsetmacro{\ex}{2*pi*\r}
  \coordinate (E) at ({\ex},0);
  % axes
  \draw[line width=0.7pt,-latex] (-1.6,0) -- ({\ex+1.3},0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-0.35) -- (0,{2*\r+0.85}) node[anchor=south,inner sep=2pt] {$y$};
  % cycloid (partial previous + one arch + a bit)
  \draw[line width=0.85pt] plot[domain=-0.55:6.55,samples=120,smooth]
    ({\r*(\x-sin(\x r))},{\r*(1-cos(\x r))});
  % rolling circle
  \draw[line width=0.7pt] (C) circle (\r);
  \draw[line width=0.6pt] (C) -- (A);
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.5pt] (C) -- (P);
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.5pt] (P) -- (Q);
  \draw[line width=0.55pt] (P) -- (M);
  % angle θ at C between downward vertical and CP
  \draw[line width=0.4pt] (C) ++(0,-0.55) arc[start angle=-90,end angle={-90-\th r},radius=0.55];
  \node[anchor=north,inner sep=1pt,font=\small] at ({(\cx)-0.35},{\r-0.72}) {$\theta$};
  \node[anchor=south east,inner sep=1pt,font=\small] at ({(\px+\cx)/2},{(\py+\r)/2+0.05}) {$r$};
  \fill (O) circle (1.15pt);
  \fill (C) circle (1.15pt);
  \fill (A) circle (1.1pt);
  \fill (P) circle (1.25pt);
  \fill (M) circle (1.05pt);
  \fill (Q) circle (1.05pt);
  \fill (E) circle (1.05pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.08,-0.08) {$O$};
...
\centering
\fitfig{%
% fig-alt: 图6·20（2） 摆线作齿条齿廓：滚动圆两位置 C，定点 P、Q，下方阴影齿槽轮廓标「摆线」
\begin{tikzpicture}[x=0.5cm,y=0.5cm,line cap=round,line join=round,>=latex]
  \def\r{1.4}
  % left circle nearly starting; right further along
  \def\thP{1.15}
  \def\thQ{2.35}
  \pgfmathsetmacro{\cpx}{\r*\thP}
  \pgfmathsetmacro{\cqx}{\r*\thQ}
  \pgfmathsetmacro{\px}{\r*(\thP-sin(\thP r))}
  \pgfmathsetmacro{\py}{\r*(1-cos(\thP r))}
  \pgfmathsetmacro{\qx}{\r*(\thQ-sin(\thQ r))}
  \pgfmathsetmacro{\qy}{\r*(1-cos(\thQ r))}
  \coordinate (O) at (0,0);
  \coordinate (Cp) at ({\cpx},\r);
  \coordinate (Cq) at ({\cqx},\r);
  \coordinate (M) at ({\cpx},0);
  \coordinate (A) at ({\cqx},0);
  \coordinate (P) at ({\px},{\py});
  \coordinate (Q) at ({\qx},{\qy});
  \pgfmathsetmacro{\Qp}{{\cpx}} % for horizontal from P to vertical CM
  \coordinate (Hp) at ({\cpx},{\py});
  \coordinate (Hq) at ({\cqx},{\qy});
  % axes
  \draw[line width=0.65pt,-latex] (-2.2,0) -- ({2*pi*\r+1.0},0);
  \draw[line width=0.65pt,-latex] (0,-2.6) -- (0,{2*\r+0.55}) node[anchor=south,inner sep=1.5pt] {$y$};
  % cycloid above (generating curve)
  \draw[line width=0.8pt] plot[domain=-0.2:6.5,samples=110,smooth]
    ({\r*(\x-sin(\x r))},{\r*(1-cos(\x r))});
  % rack tooth troughs below pitch line: inverted cycloid arches with hatch on material
  % material is OUTSIDE the troughs (below the trough curve)
  \begin{scope}
    \clip (-2.0,-2.45) rectangle ({2*pi*\r+0.3},0.02);
    % fill material (everything below pitch line except the "air" in troughs is hatched via path)
    \fill[pattern=north east lines,pattern color=black!50]
      (-2.0,0) -- (-2.0,-2.45) -- ({2*pi*\r+0.3},-2.45) -- ({2*pi*\r+0.3},0)
      -- plot[domain={2*pi}:0,samples=90,smooth] ({\r*(\x-sin(\x r))},{-0.72*\r*(1-cos(\x r))})
      -- cycle;
  \end{scope}
  \draw[line width=0.7pt]
    plot[domain=-0.15:6.4,samples=90,smooth]
    ({\r*(\x-sin(\x r))},{-0.72*\r*(1-cos(\x r))});
  % two rolling circles
  \draw[line width=0.65pt] (Cp) circle (\r);
  \draw[line width=0.65pt] (Cq) circle (\r);
  \draw[line width=0.5pt] (Cp) -- (M);
  \draw[line width=0.5pt] (Cq) -- (A);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.45pt] (Cp) -- (P);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.45pt] (P) -- (Hp);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.45pt] (Cq) -- (Q);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.45pt] (Q) -- (Hq);
  \fill (O) circle (1.1pt);
  \fill (Cp) circle (1.1pt);
  \fill (Cq) circle (1.1pt);
  \fill (P) circle (1.15pt);
  \fill (Q) circle (1.15pt);
  \fill (M) circle (1.0pt);
  \fill (A) circle (1.0pt);
  \node[anchor=north east,inner sep=1.2pt] at (-0.08,-0.08) {$O$};
  \node[anchor=south,inner sep=1.2pt] at (Cp) {$C$};
  \node[anchor=south,inner sep=1.2pt] at (Cq) {$C$};
  ...
  \node[anchor=east,inner sep=1pt,font=\small] at (-0.2,-1.0) {摆线};
  \draw[line width=0.35pt] (-0.05,-1.0) -- ({0.35*\r},{-0.35*\r});
\end{tikzpicture}%
}
\caption{图6·20（2）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图6·21（1） 圆的渐开线：基圆半径 r，切点 B，切线段 BP=弧 BA，∠AOB=θ，垂足 D、M 与水平辅助点 Q
\begin{tikzpicture}[x=0.48cm,y=0.48cm,line cap=round,line join=round,>=latex]
  \def\r{1.7}
  \def\th{1.05} % radians for main point
  \pgfmathsetmacro{\bx}{\r*cos(\th r)}
  \pgfmathsetmacro{\by}{\r*sin(\th r)}
  \pgfmathsetmacro{\px}{\r*(cos(\th r)+\th*sin(\th r))}
  \pgfmathsetmacro{\py}{\r*(sin(\th r)-\th*cos(\th r))}
  \coordinate (O) at (0,0);
  \coordinate (A) at (\r,0);
  \coordinate (B) at ({\bx},{\by});
  \coordinate (P) at ({\px},{\py});
  \coordinate (D) at ({\bx},0);
  \coordinate (M) at ({\px},0);
  \coordinate (Q) at ({\bx},{\py});
  % axes (dashed style common in this book figure)
  \draw[dash pattern=on 2.0pt off 1.3pt,line width=0.55pt,-latex] ({-1.15*\r},0) -- ({3.6*\r},0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[dash pattern=on 2.0pt off 1.3pt,line width=0.55pt,-latex] (0,{-1.15*\r}) -- (0,{2.7*\r}) node[anchor=south,inner sep=2pt] {$y$};
  % base circle
  \draw[line width=0.75pt] (0,0) circle (\r);
  % involute
  \draw[line width=0.85pt] plot[domain=0.02:4.4,samples=140,smooth]
    ({\r*(cos(\x r)+\x*sin(\x r))},{\r*(sin(\x r)-\x*cos(\x r))});
  % construction tangents at several angles
  \foreach \t in {0.45,0.75,1.35,1.75,2.2,2.7,3.2,3.7} {
    \pgfmathsetmacro{\tx}{\r*cos(\t r)}
    \pgfmathsetmacro{\ty}{\r*sin(\t r)}
    \pgfmathsetmacro{\ix}{\r*(cos(\t r)+\t*sin(\t r))}
    \pgfmathsetmacro{\iy}{\r*(sin(\t r)-\t*cos(\t r))}
    \draw[line width=0.45pt] ({\tx},{\ty}) -- ({\ix},{\iy});
    \draw[line width=0.35pt] (O) -- ({\tx},{\ty});
  }
  % main OB, BP, projections
  \draw[line width=0.7pt] (O) -- (B);
  \draw[line width=0.7pt] (B) -- (P);
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.5pt] (B) -- (D);
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.5pt] (P) -- (M);
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.5pt] (P) -- (Q);
  % right angle mark at B (OB ⊥ BP): OB dir (cos,sin), BP dir (sin,-cos)
  \draw[line width=0.35pt] (B) ++({0.28*cos(\th r)},{0.28*sin(\th r)})
    -- ++({0.28*sin(\th r)},{-0.28*cos(\th r)})
    -- ++({-0.28*cos(\th r)},{-0.28*sin(\th r)});
  % angles θ
  \draw[line width=0.4pt] (0.55,0) arc[start angle=0,end angle={\th r},radius=0.55];
  \node[anchor=west,inner sep=1pt,font=\small] at (0.7,0.22) {$\theta$};
  \draw[line width=0.35pt] (B) ++({0.5*sin(\th r)},{-0.5*cos(\th r)})
    arc[start angle={\th r-90},end angle={\th r-90+\th r},radius=0.5];
  \node[anchor=north west,inner sep=0.5pt,font=\small] at ({(\bx)+0.15},{(\py)+0.15}) {$\theta$};
  \node[anchor=north east,inner sep=1pt,font=\small] at ({0.55*\bx},{0.55*\by}) {$r$};
  \fill (O) circle (1.15pt);
  \fill (A) circle (1.1pt);
  \fill (B) circle (1.2pt);
  \fill (P) circle (1.25pt);
  \fill (D) circle (1.05pt);
  \fill (M) circle (1.05pt);
  \fill (Q) circle (1.05pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.12,-0.12) {$O$};
  \node[anchor=north,inner sep=1.5pt] at (A) {$A$};
  \node[anchor=south east,inner sep=1.5pt] at (B) {$B$};
...
\centering
\fitfig{%
% fig-alt: 图6·21（2） 齿轮齿面为圆的渐开线，虚线基圆；三齿扇形与径向点划中心线
\begin{tikzpicture}[x=1.0cm,y=1.0cm,line cap=round,line join=round,>=latex]
  \def\rb{1.45}
  \def\rr{1.2}
  \def\rt{2.05}
  \def\ox{0.16}
  \def\oy{0.12}
  % radial dash-dot lines for tooth centers
  \foreach \ang in {70,90,110} {
    \draw[dash pattern=on 2.0pt off 1.1pt on 0.55pt off 1.1pt,line width=0.4pt]
      (0,0) -- ({1.15*\rt*cos(\ang)},{1.15*\rt*sin(\ang)});
  }
  % back face arcs (light offset)
  \begin{scope}[shift={(\ox,\oy)}]
    \draw[line width=0.4pt,black!45] ({\rr*cos(55)},{\rr*sin(55)}) arc[start angle=55,end angle=125,radius=\rr];
    \draw[line width=0.4pt,black!45] ({\rt*cos(62)},{\rt*sin(62)}) arc[start angle=62,end angle=78,radius=\rt];
    \draw[line width=0.4pt,black!45] ({\rt*cos(82)},{\rt*sin(82)}) arc[start angle=82,end angle=98,radius=\rt];
    \draw[line width=0.4pt,black!45] ({\rt*cos(102)},{\rt*sin(102)}) arc[start angle=102,end angle=118,radius=\rt];
  \end{scope}
  % root arc
  \draw[line width=0.7pt] ({\rr*cos(55)},{\rr*sin(55)}) arc[start angle=55,end angle=125,radius=\rr];
  % tip lands
  \foreach \a/\b in {63/77,83/97,103/117} {
    \draw[line width=0.7pt] ({\rt*cos(\a)},{\rt*sin(\a)}) arc[start angle=\a,end angle=\b,radius=\rt];
  }
  % base circle
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.55pt]
    ({\rb*cos(52)},{\rb*sin(52)}) arc[start angle=52,end angle=128,radius=\rb];
  % involute flanks for three teeth
  \foreach \phi in {70,90,110} {
    % left flank
    \draw[line width=0.72pt] plot[domain=0.12:0.95,samples=30,smooth]
      ({(\rb*(cos(\x r)+\x*sin(\x r)))*cos(\phi+9)-(\rb*(sin(\x r)-\x*cos(\x r)))*sin(\phi+9)},
       {(\rb*(cos(\x r)+\x*sin(\x r)))*sin(\phi+9)+(\rb*(sin(\x r)-\x*cos(\x r)))*cos(\phi+9)});
    % right flank
    \draw[line width=0.72pt] plot[domain=0.12:0.95,samples=30,smooth]
      ({(\rb*(cos(\x r)+\x*sin(\x r)))*cos(\phi-9)+(\rb*(sin(\x r)-\x*cos(\x r)))*sin(\phi-9)},
       {(\rb*(cos(\x r)+\x*sin(\x r)))*sin(\phi-9)-(\rb*(sin(\x r)-\x*cos(\x r)))*cos(\phi-9)});
  }
  % root fillets connecting tip gaps (approx chords at root between teeth)
  \foreach \a/\b in {77/83,97/103} {
    \draw[line width=0.55pt] ({\rr*1.02*cos(\a)},{\rr*1.02*sin(\a)}) -- ({\rr*1.02*cos(\b)},{\rr*1.02*sin(\b)});
  }
  % thickness connectors
  \draw[line width=0.4pt,black!50] ({\rt*cos(63)},{\rt*sin(63)}) -- ({\rt*cos(63)+\ox},{\rt*sin(63)+\oy});
  \draw[line width=0.4pt,black!50] ({\rr*cos(55)},{\rr*sin(55)}) -- ({\rr*cos(55)+\ox},{\rr*sin(55)+\oy});
  % dotted extension of one involute
  \draw[dash pattern=on 1.1pt off 0.9pt,line width=0.4pt] plot[domain=0.85:1.2,samples=18,smooth]
    ({(\rb*(cos(\x r)+\x*sin(\x r)))*cos(79)-(\rb*(sin(\x r)-\x*cos(\x r)))*sin(79)},
     {(\rb*(cos(\x r)+\x*sin(\x r)))*sin(79)+(\rb*(sin(\x r)-\x*cos(\x r)))*cos(79)});
  \node[anchor=east,inner sep=1pt,font=\small] at (-0.05,1.4) {基圆};
  \draw[line width=0.35pt] (0.05,1.4) -- ({\rb*cos(105)},{\rb*sin(105)});
  \node[anchor=south,inner sep=1pt,font=\small] at (0.2,2.35) {渐开线};
  \draw[line width=0.35pt] (0.35,2.25) -- ({1.7*cos(100)},{2.05*sin(100)});
  \fill (0,0) circle (1.15pt);
\end{tikzpicture}%
}
\caption{图6·21（2）}
...
\centering
\fitfig{%
% fig-alt: 图6·22 底边 AB=2a、高 h 的三角形，垂心 P 的轨迹为抛物线；C(t,h)、高 CD 与 BE⊥AC
\begin{tikzpicture}[x=0.72cm,y=0.72cm,line cap=round,line join=round,>=latex]
  \def\a{2.35}
  \def\h{2.75}
  \def\t{1.15}
  \pgfmathsetmacro{\py}{(\a*\a-\t*\t)/\h}
  \coordinate (O) at (0,0);
  \coordinate (A) at ({\a},0);
  \coordinate (B) at ({-\a},0);
  \coordinate (C) at ({\t},{\h});
  ...
  \coordinate (P) at ({\t},{\py});
  \pgfmathsetmacro{\ta}{\t-\a}
  \pgfmathsetmacro{\xe}{\a*(\h*\h-\ta*\ta)/(\h*\h+\ta*\ta)}
  \pgfmathsetmacro{\ye}{(\h/\ta)*(\xe-\a)}
  \coordinate (E) at ({\xe},{\ye});
  \draw[line width=0.7pt,-latex] ({-\a-0.7},0) -- ({\a+1.0},0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-1.15) -- (0,{\h+0.65}) node[anchor=south,inner sep=2pt] {$y$};
  \draw[dash pattern=on 2.0pt off 1.3pt,line width=0.5pt] ({-\a-0.35},{\h}) -- ({\a+0.55},{\h});
  \draw[line width=0.8pt] plot[domain={-\a-0.4}:{\a+0.4},samples=70,smooth]
    ({\x},{(\a*\a-(\x)*(\x))/\h});
  \draw[line width=0.7pt] (B) -- (C) -- (A);
  \draw[line width=0.65pt] (C) -- (D);
  \draw[line width=0.65pt] (B) -- (E);
  % right angle BE ⊥ AC at E
  \pgfmathsetmacro{\acx}{\t-\a}
  \pgfmathsetmacro{\acy}{\h}
  \pgfmathsetmacro{\acl}{sqrt(\acx*\acx+\acy*\acy)}
  \pgfmathsetmacro{\ux}{\acx/\acl}
  \pgfmathsetmacro{\uy}{\acy/\acl}
  \pgfmathsetmacro{\bex}{-\a-\xe}
  \pgfmathsetmacro{\bey}{0-\ye}
  \pgfmathsetmacro{\bel}{sqrt(\bex*\bex+\bey*\bey)}
  \pgfmathsetmacro{\vx}{\bex/\bel}
  \pgfmathsetmacro{\vy}{\bey/\bel}
  % unit along AC toward A from E, and along EB toward B
  \draw[line width=0.35pt] (E) ++({-0.24*\ux},{-0.24*\uy}) -- ++({0.24*\vx},{0.24*\vy}) -- ++({0.24*\ux},{0.24*\uy});
  \fill (O) circle (1.1pt);
  \fill (A) circle (1.1pt);
  \fill (B) circle (1.1pt);
  \fill (C) circle (1.15pt);
  ...
  \fill (E) circle (1.05pt);
  \node[anchor=north east,inner sep=1pt] at (-0.1,-0.1) {$O$};
  \node[anchor=north,inner sep=1.5pt] at (A) {$A(a,0)$};
  \node[anchor=north,inner sep=1.5pt] at (B) {$B(-a,0)$};
  \node[anchor=south west,inner sep=1.5pt] at (C) {$C(t,h)$};
  \node[anchor=north west,inner sep=1.5pt] at (D) {$D$};
  \node[anchor=west,inner sep=2pt] at (P) {$P$};
...
\centering
\fitfig{%
% fig-alt: 图6·23 椭圆的两条垂直切线交于点 C，交点轨迹为圆 x^2+y^2=a^2+b^2
\begin{tikzpicture}[x=0.7cm,y=0.7cm,line cap=round,line join=round,>=latex]
  \def\a{2.25}
  \def\b{1.35}
  \pgfmathsetmacro{\R}{sqrt(\a*\a+\b*\b)}
  \def\m{0.75}
  \pgfmathsetmacro{\sone}{sqrt(\a*\a*\m*\m+\b*\b)}
  \pgfmathsetmacro{\stwo}{sqrt(\a*\a/(\m*\m)+\b*\b)}
  \pgfmathsetmacro{\cx}{(\stwo-\sone)/(\m+1/\m)}
  \pgfmathsetmacro{\cy}{\m*\cx+\sone}
  \coordinate (O) at (0,0);
  \coordinate (C) at ({\cx},{\cy});
  \draw[line width=0.7pt,-latex] ({-\R-0.35},0) -- ({\R+0.55},0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,{-\R-0.3}) -- (0,{\R+0.5}) node[anchor=south,inner sep=2pt] {$y$};
  \draw[line width=0.75pt] (0,0) ellipse ({\a} and {\b});
  \draw[line width=0.7pt] (0,0) circle ({\R});
  % two perpendicular tangents through C, extended
  \draw[line width=0.65pt]
    ({\cx-2.8},{\cy-\m*2.8}) -- ({\cx+1.6},{\cy+\m*1.6});
  \draw[line width=0.65pt]
    ({\cx-1.3},{\cy+(1/\m)*1.3}) -- ({\cx+2.2},{\cy-(1/\m)*2.2});
  \fill (O) circle (1.1pt);
  \fill (C) circle (1.25pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.1,-0.1) {$O$};
...
\centering
\fitfig{%
% fig-alt: 图6·24 椭圆上点 P 的切线交 x 轴于 Q，△OPQ 垂心 H 的轨迹为另一椭圆；垂线 HD 过 P
\begin{tikzpicture}[x=0.72cm,y=0.72cm,line cap=round,line join=round,>=latex]
  % original ellipse: x=2cosθ, y=sinθ → a=2,b=1
  % H locus: x=2cosθ, y=4sinθ → a=2,b=4
  \def\ae{2.0}
  \def\be{1.0}
  \def\bh{3.2} % draw locus a bit compressed for fit (ratio similar)
  \def\th{48} % degrees for illustration
  \pgfmathsetmacro{\px}{\ae*cos(\th)}
  \pgfmathsetmacro{\py}{\be*sin(\th)}
  \pgfmathsetmacro{\hx}{\ae*cos(\th)}
  \pgfmathsetmacro{\hy}{\bh*sin(\th)}
  \pgfmathsetmacro{\qx}{\ae/cos(\th)}
  \coordinate (O) at (0,0);
  \coordinate (P) at ({\px},{\py});
  \coordinate (H) at ({\hx},{\hy});
  \coordinate (D) at ({\px},0);
  \coordinate (Q) at ({\qx},0);
  \draw[line width=0.7pt,-latex] (-2.6,0) -- (3.4,0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-3.5) -- (0,3.7) node[anchor=south,inner sep=2pt] {$y$};
  % original (flat) ellipse
  \draw[line width=0.75pt] (0,0) ellipse ({\ae} and {\be});
  % H locus (tall) ellipse
  \draw[line width=0.75pt] (0,0) ellipse ({\ae} and {\bh});
  % lines
  \draw[line width=0.6pt] (O) -- (H);
  \draw[line width=0.55pt] (H) -- (D);
  \draw[line width=0.55pt] (P) -- (Q);
  \draw[line width=0.5pt] (O) -- (P);
  \fill (O) circle (1.1pt);
  \fill (P) circle (1.15pt);
  \fill (H) circle (1.2pt);
  \fill (D) circle (1.05pt);
  \fill (Q) circle (1.1pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.1,-0.1) {$O$};
  \node[anchor=west,inner sep=2pt] at (H) {$H(x,y)$};
...
\centering
\fitfig{%
% fig-alt: 图6·25（a） 圆的一组平行弦（水平）及其中点落在竖直直径上
\begin{tikzpicture}[x=0.85cm,y=0.85cm,line cap=round,line join=round]
  ...
  \foreach \k in {-9,-8,...,9} {
    \pgfmathsetmacro{\yy}{\k*\R/10}
    \pgfmathsetmacro{\xx}{sqrt(\R*\R-\yy*\yy)}
    \draw[line width=0.55pt] ({-\xx},{\yy}) -- ({\xx},{\yy});
    \draw[line width=0.55pt,fill=white] (0,{\yy}) circle (0.07);
  }
  \draw[line width=0.55pt,fill=white] (0,{\R}) circle (0.07);
  \draw[line width=0.55pt,fill=white] (0,{-\R}) circle (0.07);
\end{tikzpicture}%
}
\caption{(a)}
...
\centering
\fitfig{%
% fig-alt: 图6·25（b） 椭圆内一组斜率为负的平行弦，中点落在过原点的直径 P₁P₂ 上；弦 UV 中点为 P′
\begin{tikzpicture}[x=0.72cm,y=0.72cm,line cap=round,line join=round,>=latex]
  \def\a{2.55}
  \def\b{1.65}
  \def\k{-0.85}
  \pgfmathsetmacro{\mm}{-\b*\b/(\a*\a*\k)}
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.45pt,-latex] ({-\a-0.55},0) -- ({\a+0.7},0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.45pt,-latex] (0,{-\b-0.45}) -- (0,{\b+0.65}) node[anchor=south,inner sep=2pt] {$y$};
  \pgfmathsetmacro{\den}{sqrt(1/(\a*\a)+\mm*\mm/(\b*\b))}
  \pgfmathsetmacro{\px}{1/\den}
  \pgfmathsetmacro{\py}{\mm/\den}
  \coordinate (O) at (0,0);
  \coordinate (P1) at ({-\px},{-\py});
  \coordinate (P2) at ({\px},{\py});
  \draw[line width=0.85pt] (0,0) ellipse ({\a} and {\b});
  \draw[line width=0.75pt] (P1) -- (P2);
  \foreach \t in {-0.82,-0.62,-0.42,-0.22,0.22,0.42,0.62,0.82} {
    \pgfmathsetmacro{\mx}{\t*\px}
    \pgfmathsetmacro{\my}{\t*\py}
    \pgfmathsetmacro{\coff}{\my - \k*\mx}
    \pgfmathsetmacro{\AA}{\b*\b + \a*\a*\k*\k}
    \pgfmathsetmacro{\BB}{2*\a*\a*\k*\coff}
    \pgfmathsetmacro{\CC}{\a*\a*(\coff*\coff - \b*\b)}
    \pgfmathsetmacro{\sd}{sqrt(\BB*\BB - 4*\AA*\CC)}
    \pgfmathsetmacro{\xa}{(-\BB+\sd)/(2*\AA)}
    \pgfmathsetmacro{\xb}{(-\BB-\sd)/(2*\AA)}
    \pgfmathsetmacro{\ya}{\k*\xa+\coff}
    \pgfmathsetmacro{\yb}{\k*\xb+\coff}
    \draw[line width=0.55pt] ({\xa},{\ya}) -- ({\xb},{\yb});
    \draw[line width=0.5pt,fill=white] ({\mx},{\my}) circle (0.08);
  }
  % labeled chord UV at t=0.28
  \def\tu{0.28}
  \pgfmathsetmacro{\umx}{\tu*\px}
  \pgfmathsetmacro{\umy}{\tu*\py}
  \pgfmathsetmacro{\ucoff}{\umy - \k*\umx}
  \pgfmathsetmacro{\uA}{\b*\b + \a*\a*\k*\k}
  \pgfmathsetmacro{\uB}{2*\a*\a*\k*\ucoff}
  \pgfmathsetmacro{\uC}{\a*\a*(\ucoff*\ucoff - \b*\b)}
  \pgfmathsetmacro{\usd}{sqrt(\uB*\uB - 4*\uA*\uC)}
  \pgfmathsetmacro{\uxone}{(-\uB+\usd)/(2*\uA)}
  \pgfmathsetmacro{\uxtwo}{(-\uB-\usd)/(2*\uA)}
  \pgfmathsetmacro{\uyone}{\k*\uxone+\ucoff}
  \pgfmathsetmacro{\uytwo}{\k*\uxtwo+\ucoff}
  \coordinate (U) at ({\uxtwo},{\uytwo});
  \coordinate (V) at ({\uxone},{\uyone});
  \coordinate (Pp) at ({\umx},{\umy});
  \draw[line width=0.7pt] (U) -- (V);
  \draw[line width=0.5pt,fill=white] (Pp) circle (0.085);
  \fill (O) circle (1.1pt);
  \node[anchor=north east,inner sep=1.2pt] at (-0.08,-0.08) {$O$};
  \node[anchor=north east,inner sep=1.2pt] at (P1) {$P_1$};
  \node[anchor=south west,inner sep=1.2pt] at (P2) {$P_2$};
  \node[anchor=south east,inner sep=1.2pt] at (U) {$U$};
  \node[anchor=north west,inner sep=1.2pt] at (V) {$V$};
  \node[anchor=west,inner sep=1.5pt] at (Pp) {$P'$};
\end{tikzpicture}%
}
\caption{(b)}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图6·26（a） 双曲线一组平行弦（虚线）及其中点落在过原点的直径上
\begin{tikzpicture}[x=0.52cm,y=0.52cm,line cap=round,line join=round,>=latex]
  \def\a{1.6}
  \def\b{1.35}
  \def\k{1.35}
  \pgfmathsetmacro{\mm}{\b*\b/(\a*\a*\k)}
  \draw[line width=0.7pt,-latex] (-4.6,0) -- (4.8,0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-3.6) -- (0,3.8) node[anchor=south,inner sep=2pt] {$y$};
  % asymptotes (thin)
  \draw[line width=0.35pt] ({-4.0},{-4.0*\b/\a}) -- ({4.0},{4.0*\b/\a});
  \draw[line width=0.35pt] ({-4.0},{4.0*\b/\a}) -- ({4.0},{-4.0*\b/\a});
  % hyperbola
  \draw[line width=0.85pt,domain=-1.45:1.45,samples=55,smooth]
    plot ({\a*cosh(\x)},{\b*sinh(\x)});
  \draw[line width=0.85pt,domain=-1.45:1.45,samples=55,smooth]
    plot ({-\a*cosh(\x)},{\b*sinh(\x)});
  % diameter
  \draw[line width=0.75pt] ({-3.5},{-3.5*\mm}) -- ({3.5},{3.5*\mm});
  % parallel chords through midpoints on diameter
  \foreach \s in {-1.55,-0.95,-0.4,0.4,0.95,1.55} {
    \pgfmathsetmacro{\mx}{\s}
    \pgfmathsetmacro{\my}{\mm*\s}
    \pgfmathsetmacro{\coff}{\my - \k*\mx}
    % solve (b^2 - a^2 k^2) x^2 - 2 a^2 k coff x - a^2 coff^2 - a^2 b^2 = 0
    \pgfmathsetmacro{\AA}{\b*\b - \a*\a*\k*\k}
    \pgfmathsetmacro{\BB}{-2*\a*\a*\k*\coff}
    \pgfmathsetmacro{\CC}{-\a*\a*\coff*\coff - \a*\a*\b*\b}
    \pgfmathsetmacro{\Delta}{\BB*\BB - 4*\AA*\CC}
    \pgfmathsetmacro{\sd}{sqrt(max(\Delta,0))}
    \pgfmathsetmacro{\xa}{(-\BB+\sd)/(2*\AA)}
    \pgfmathsetmacro{\xb}{(-\BB-\sd)/(2*\AA)}
    \pgfmathsetmacro{\ya}{\k*\xa+\coff}
    \pgfmathsetmacro{\yb}{\k*\xb+\coff}
    \draw[dash pattern=on 1.7pt off 1.1pt,line width=0.5pt]
      ({\xa},{\ya}) -- ({\xb},{\yb});
    \fill ({\mx},{\my}) circle (1.2pt);
  }
  \fill (0,0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.1,-0.1) {$O$};
\end{tikzpicture}%
...
\centering
\fitfig{%
% fig-alt: 图6·26（b） 抛物线一组斜率为正的平行弦（虚线），中点落在水平直径上
\begin{tikzpicture}[x=0.55cm,y=0.55cm,line cap=round,line join=round,>=latex]
  \def\pp{1.35}
  \def\k{0.95}
  \pgfmathsetmacro{\yd}{\pp/\k}
  \draw[line width=0.7pt,-latex] (-0.6,0) -- (5.2,0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-3.3) -- (0,3.5) node[anchor=south,inner sep=2pt] {$y$};
  \draw[line width=0.85pt,domain=-3.1:3.1,samples=70,smooth]
    plot ({(\x*\x)/(2*\pp)},{\x});
  \draw[line width=0.7pt] (0.05,{\yd}) -- (4.7,{\yd});
  \foreach \mx in {1.0,1.7,2.4,3.1,3.8} {
    \pgfmathsetmacro{\coff}{\yd - \k*\mx}
    \pgfmathsetmacro{\AA}{\k*\k}
    \pgfmathsetmacro{\BB}{2*\k*\coff - 2*\pp}
    \pgfmathsetmacro{\CC}{\coff*\coff}
    \pgfmathsetmacro{\sd}{sqrt(\BB*\BB - 4*\AA*\CC)}
    \pgfmathsetmacro{\xa}{(-\BB+\sd)/(2*\AA)}
    \pgfmathsetmacro{\xb}{(-\BB-\sd)/(2*\AA)}
    \pgfmathsetmacro{\ya}{\k*\xa+\coff}
...
\centering
\fitfig{%
% fig-alt: 第1题 刚体棒 AB 两端在坐标轴上滑动，P 分 AB 使 BP=a、PA=b，夹角 θ，垂足 M、N
\begin{tikzpicture}[x=0.72cm,y=0.72cm,line cap=round,line join=round,>=latex]
  \def\aa{2.35}
  \def\bb{1.85}
  \def\th{38}
  % A on x-axis, B on y-axis; AB makes angle th with x-axis
  % length AB = a+b; A=( (a+b) cos th, 0), B=(0, (a+b) sin th)
  % P divides: from B along BA by distance a? BP=a, PA=b
  % From B toward A: distance a of total a+b
  \pgfmathsetmacro{\L}{\aa+\bb}
  \pgfmathsetmacro{\Ax}{\L*cos(\th)}
  \pgfmathsetmacro{\By}{\L*sin(\th)}
  \pgfmathsetmacro{\Px}{\aa*\Ax/\L}
  \pgfmathsetmacro{\Py}{\bb*\By/\L}
  % Wait: P = B + (a/(a+b))(A-B) if BP=a
  % A-B = (Ax, -By); |A-B|=L
  % P = (0,By) + (aa/L)*(Ax, -By) = (aa*Ax/L, By - aa*By/L) = (aa*Ax/L, bb*By/L)
  \coordinate (O) at (0,0);
  \coordinate (A) at ({\Ax},0);
  \coordinate (B) at (0,{\By});
  \coordinate (P) at ({\aa*\Ax/\L},{\bb*\By/\L});
  \coordinate (M) at ({\aa*\Ax/\L},0);
  \coordinate (N) at (0,{\bb*\By/\L});
  \draw[line width=0.7pt,-latex] (-0.45,0) -- ({\Ax+0.85},0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-0.4) -- (0,{\By+0.75}) node[anchor=south,inner sep=2pt] {$y$};
  \draw[line width=0.85pt] (B) -- (A);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.5pt] (P) -- (M);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.5pt] (P) -- (N);
  % angle θ at A
  \draw[line width=0.4pt] (A) ++(180:0.55) arc[start angle=180,end angle={180-\th},radius=0.55];
  \node[anchor=east,inner sep=1pt,font=\small] at ({(\Ax)-0.62},{0.28}) {$\theta$};
  \fill (O) circle (1.1pt);
  \fill (P) circle (1.25pt);
  ...
  \node[anchor=north,inner sep=1.5pt] at (M) {$M$};
  \node[anchor=east,inner sep=1.5pt] at (N) {$N$};
  \node[anchor=south east,inner sep=1pt,font=\small] at ({(\aa*\Ax/\L)/2},{(\By+\bb*\By/\L)/2}) {$a$};
  \node[anchor=south west,inner sep=1pt,font=\small] at ({(\aa*\Ax/\L+\Ax)/2},{(\bb*\By/\L)/2}) {$b$};
\end{tikzpicture}%
}
\caption{(第1题)}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 第2题 过定点 Q(a,b) 的直线交两轴于 A、B，AB 中点 P，垂足 M、N
\begin{tikzpicture}[x=0.65cm,y=0.65cm,line cap=round,line join=round,>=latex]
  % Choose A=(4.2,0), B=(0,2.8); line extends to Q in Q2
  \coordinate (O) at (0,0);
  \coordinate (A) at (4.2,0);
  \coordinate (B) at (0,2.8);
  \coordinate (P) at (2.1,1.4);
  \coordinate (M) at (2.1,0);
  \coordinate (N) at (0,1.4);
  % Q on extension beyond B: parametric from A through B
  % direction A->B = (-4.2, 2.8); beyond B: B + t(B-A)/|...|
  % B-A = (-4.2,2.8); unit-ish: place Q at B + 0.55*(B-A)/something
  \coordinate (Q) at (-1.7,3.9);
  \draw[line width=0.7pt,-latex] (-2.3,0) -- (5.0,0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-0.45) -- (0,4.5) node[anchor=south,inner sep=2pt] {$y$};
  \draw[line width=0.8pt] (Q) -- (A);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.5pt] (P) -- (M);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.5pt] (P) -- (N);
  \fill (O) circle (1.1pt);
  \fill (P) circle (1.25pt);
  ...
  \node[anchor=east,inner sep=1.5pt] at (B) {$B$};
  \node[anchor=south west,inner sep=1.5pt] at (P) {$P$};
  \node[anchor=south east,inner sep=1.5pt] at (Q) {$Q(a,b)$};
  \node[anchor=north,inner sep=1.5pt] at (M) {$M$};
  \node[anchor=east,inner sep=1.5pt] at (N) {$N$};
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: 第10题 圆 x²+(y-a)²=a² 上动点 B，OB 延长交 y=2a 于 P，PD⊥x 轴，Q 为 B 在 PD 上的射影
\begin{tikzpicture}[x=0.7cm,y=0.7cm,line cap=round,line join=round,>=latex]
  \def\aa{2.0}
  \def\th{42}
  \coordinate (O) at (0,0);
  \coordinate (A) at (0,{2*\aa});
  % center of circle (0,a)
  \coordinate (C0) at (0,{\aa});
  % P on y=2a: P=(2a cot? from O at angle th: line y = x tan th, intersect y=2a => x=2a/tan=2a cot
  \pgfmathsetmacro{\Px}{2*\aa / tan(\th)}
  \coordinate (P) at ({\Px},{2*\aa});
  \coordinate (D) at ({\Px},0);
  % B = intersection of OP with circle (besides possibly O)
  % circle: x^2 + (y-a)^2 = a^2 => x^2 + y^2 - 2ay = 0
  % line: y = x tan th
  % x^2 + x^2 tan^2 - 2a x tan = 0 => x(x(1+tan^2) - 2a tan)=0
  % x = 0 or x = 2a tan / (1+tan^2) = a sin(2th)? = a * 2sin cos /1? 
  % 2a tan/(1+tan^2) = 2a (s/c) / (1/c^2) = 2a s c = a sin 2θ
  \pgfmathsetmacro{\Bx}{\aa * sin(2*\th)}
  \pgfmathsetmacro{\By}{\Bx * tan(\th)}
  \coordinate (B) at ({\Bx},{\By});
  \coordinate (E) at (0,{\By});
  \coordinate (Q) at ({\Px},{\By});
  \draw[line width=0.7pt,-latex] (-2.4,0) -- ({\Px+0.9},0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-0.4) -- (0,{2*\aa+0.85}) node[anchor=south,inner sep=2pt] {$y$};
  \draw[line width=0.8pt] (C0) circle (\aa);
  \draw[line width=0.55pt] ({-0.35},{2*\aa}) -- ({\Px+0.35},{2*\aa});
  \draw[line width=0.75pt] (O) -- (P);
  \draw[line width=0.55pt] (P) -- (D);
  \draw[line width=0.5pt] (B) -- (E);
  \draw[line width=0.5pt] (B) -- (Q);
  \draw[dash pattern=on 1.5pt off 1.0pt,line width=0.45pt] (A) -- (B);
  % angle θ
  \draw[line width=0.4pt] (0.55,0) arc[start angle=0,end angle=\th,radius=0.55];
  \node[anchor=west,inner sep=1pt,font=\small] at (0.62,0.22) {$\theta$};
  \fill (O) circle (1.1pt);
  \fill (A) circle (1.1pt);
  \fill (B) circle (1.15pt);
  \fill (P) circle (1.15pt);
...
\centering
\fitfig{%
% fig-alt: 第11题 大圆半径 4r 内小圆半径 r 滚动，定点 P 描四歧点内摆线（星形线）；∠AOB=φ，∠BCP=4φ
\begin{tikzpicture}[x=0.42cm,y=0.42cm,line cap=round,line join=round,>=latex]
  \def\rr{1.0}
  \def\ph{32}
  \pgfmathsetmacro{\R}{4*\rr}
  \pgfmathsetmacro{\oc}{3*\rr}
  % C = (3r cos φ, 3r sin φ)
  \pgfmathsetmacro{\Cx}{\oc*cos(\ph)}
  \pgfmathsetmacro{\Cy}{\oc*sin(\ph)}
  % B = tangency = (4r cos φ, 4r sin φ)
  \pgfmathsetmacro{\Bx}{\R*cos(\ph)}
  \pgfmathsetmacro{\By}{\R*sin(\ph)}
  % P = C + r*(cos(φ - 4φ?), from text:
  % angle PCE = 90° - 3φ, with E on CD vertical?
  % x = 3r cosφ + r cos 3φ
  % y = 3r sinφ - r sin 3φ
  \pgfmathsetmacro{\Px}{\oc*cos(\ph) + \rr*cos(3*\ph)}
  \pgfmathsetmacro{\Py}{\oc*sin(\ph) - \rr*sin(3*\ph)}
  \coordinate (O) at (0,0);
  \coordinate (A) at ({\R},0);
  \coordinate (C) at ({\Cx},{\Cy});
  \coordinate (B) at ({\Bx},{\By});
  ...
  \coordinate (M) at ({\Px},0);
  \coordinate (E) at ({\Cx},{\Py});
  % axes
  \draw[dash pattern=on 1.5pt off 1.0pt,line width=0.45pt,-latex] ({-\R-0.4},0) -- ({\R+0.85},0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[dash pattern=on 1.5pt off 1.0pt,line width=0.45pt,-latex] (0,{-\R-0.35}) -- (0,{\R+0.85}) node[anchor=south,inner sep=2pt] {$y$};
  % large circle
  \draw[line width=0.7pt] (0,0) circle ({\R});
  % astroid
  \draw[line width=0.85pt,domain=0:360,samples=180,smooth]
    plot ({\R*cos(\x)*cos(\x)*cos(\x)},{\R*sin(\x)*sin(\x)*sin(\x)});
  % small circle
  \draw[line width=0.7pt] (C) circle (\rr);
  \draw[line width=0.55pt] (O) -- (B);
  \draw[line width=0.55pt] (C) -- (P);
  \draw[dash pattern=on 1.5pt off 1.0pt,line width=0.45pt] (C) -- (D);
  \draw[dash pattern=on 1.5pt off 1.0pt,line width=0.45pt] (P) -- (M);
  \draw[dash pattern=on 1.5pt off 1.0pt,line width=0.45pt] (P) -- (E);
  % angle φ at O
  \draw[line width=0.4pt] (0.7,0) arc[start angle=0,end angle=\ph,radius=0.7];
  \node[anchor=west,inner sep=1pt,font=\small] at (0.78,0.28) {$\phi$};
  % angle 4φ at C between CB and CP
  % CB direction = φ (from C toward B = outward along OC)
  % CP direction: P-C = (r cos 3φ, -r sin 3φ), angle = atan2(-sin 3φ, cos 3φ) = -3φ
  % angle from CB (direction φ) to CP (direction -3φ) = -3φ - φ = -4φ (clockwise magnitude 4φ)
  \draw[line width=0.4pt] (C) ++({\ph}:0.45) arc[start angle=\ph,end angle={-3*\ph},radius=0.45];
  \node[anchor=south,inner sep=0.5pt,font=\scriptsize] at ({(\Cx)+0.15},{(\Cy)+0.55}) {$4\phi$};
  \node[anchor=west,inner sep=1pt,font=\small] at ({(\Cx+\Bx)/2},{(\Cy+\By)/2+0.15}) {$r$};
  \node[anchor=east,inner sep=1pt,font=\small] at (-0.15,{\R*0.55}) {$4r$};
  \fill (O) circle (1.15pt);
  \fill (A) circle (1.1pt);
  \fill (B) circle (1.1pt);
  \fill (C) circle (1.15pt);
...
\centering
\fitfig{%
% fig-alt: 第12题 曲棍 OB 绕 O 旋转，连杆 AB=OB=r，A 在 x 轴滑动；P 在 AB 上，PB=b、PA=a，∠θ
\begin{tikzpicture}[x=0.7cm,y=0.7cm,line cap=round,line join=round,>=latex]
  \def\rr{2.4}
  \def\th{48}
  \def\bb{0.95}
  \def\aa{1.45}
  % ensure a+b ≈ r
  \pgfmathsetmacro{\Ax}{2*\rr*cos(\th)}
  \pgfmathsetmacro{\Bx}{\rr*cos(\th)}
  \pgfmathsetmacro{\By}{\rr*sin(\th)}
  % P = B + (b/r)(A-B); A=(Ax,0), B=(Bx,By)
  \pgfmathsetmacro{\Px}{\Bx + (\bb/\rr)*(\Ax - \Bx)}
  \pgfmathsetmacro{\Py}{\By + (\bb/\rr)*(0 - \By)}
  \coordinate (O) at (0,0);
  \coordinate (A) at ({\Ax},0);
  \coordinate (B) at ({\Bx},{\By});
  \coordinate (P) at ({\Px},{\Py});
  \coordinate (Dd) at ({\Bx},0);
  \coordinate (Mm) at ({\Px},0);
  \coordinate (Cc) at ({\Bx},{\Py});
  \draw[line width=0.7pt,-latex] (-0.5,0) -- ({\Ax+0.9},0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-0.4) -- (0,{\By+0.85}) node[anchor=south,inner sep=2pt] {$y$};
  \draw[line width=0.75pt] (0,0) circle (\rr);
  \draw[line width=0.8pt] (O) -- (B);
  \draw[line width=0.8pt] (B) -- (A);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.5pt] (B) -- (Dd);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.5pt] (P) -- (Mm);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.5pt] (P) -- (Cc);
  % angles θ
  \draw[line width=0.4pt] (0.5,0) arc[start angle=0,end angle=\th,radius=0.5];
  \node[anchor=west,inner sep=1pt,font=\small] at (0.55,0.22) {$\theta$};
  % angle at A between AB and x-axis (=θ)
  \draw[line width=0.4pt] (A) ++(180:0.45) arc[start angle=180,end angle={180-\th},radius=0.45];
  \node[anchor=east,inner sep=1pt,font=\small] at ({(\Ax)-0.55},{0.25}) {$\theta$};
  % small θ near P (angle of BP with horizontal)
  \draw[line width=0.35pt] (P) ++(180-\th:0.35) arc[start angle={180-\th},end angle=180,radius=0.35];
  \node[anchor=east,inner sep=0.5pt,font=\scriptsize] at ({(\Px)-0.4},{(\Py)+0.15}) {$\theta$};
  \fill (O) circle (1.1pt);
  \fill (A) circle (1.1pt);
  \fill (B) circle (1.15pt);
  \fill (P) circle (1.2pt);
  ...
  \node[anchor=south west,inner sep=1.5pt] at (P) {$P$};
  \node[anchor=north,inner sep=1.5pt] at (Dd) {$D$};
  \node[anchor=north,inner sep=1.5pt] at (Mm) {$M$};
  \node[anchor=east,inner sep=1pt] at (Cc) {$C$};
  \node[anchor=south east,inner sep=1pt,font=\small] at ({(\Bx)/2},{(\By)/2+0.05}) {$r$};
  \node[anchor=south east,inner sep=1pt,font=\small] at ({(\Bx+\Px)/2},{(\By+\Py)/2+0.08}) {$b$};
  \node[anchor=south,inner sep=1pt,font=\small] at ({(\Px+\Ax)/2},{(\Py)/2+0.05}) {$a$};
\end{tikzpicture}%
}
\caption{(第12题)}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 第5题 炮兵阵地 O 向倾角 15° 山坡上目标 P 发炮；发射角 θ，山脚 A，弹道抛物线交山坡于 P
\begin{tikzpicture}[x=0.78cm,y=0.78cm,line cap=round,line join=round,>=latex]
  \coordinate (O) at (0,0);
  \coordinate (A) at (3.15,0);
  % incline 15°, P on incline
  \pgfmathsetmacro{\Px}{4.55}
  \pgfmathsetmacro{\Py}{(\Px-3.15)*tan(15)}
  \coordinate (P) at ({\Px},{\Py});
  \coordinate (Aend) at ({5.55},{(5.55-3.15)*tan(15)});
  % axes
  \draw[line width=0.7pt,-latex] (-0.35,0) -- (6.15,0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-0.25) -- (0,3.55) node[anchor=south,inner sep=2pt] {$y$};
  % launch ray at θ=45°
  \draw[line width=0.55pt] (0,0) -- (1.55,1.55);
  \draw[line width=0.45pt] (0.72,0) arc (0:45:0.72);
  \node[anchor=west,inner sep=1pt,font=\small] at (0.82,0.28) {$\theta$};
  % parabola: y = x - x^2/(2*rangeish); peak then down to P
  % fit through O with initial slope 1 (45°) and through P
  % y = x + c x^2, y'(0)=1; at P: Py = Px + c Px^2 => c=(Py-Px)/Px^2
  \pgfmathsetmacro{\cc}{(\Py-\Px)/(\Px*\Px)}
  \draw[line width=0.85pt,domain=0:{\Px},samples=60,smooth]
    plot (\x,{\x+\cc*\x*\x});
  % incline with hatch
  \draw[line width=0.75pt] (A) -- (Aend);
  \draw[dash pattern=on 1.5pt off 1.1pt,line width=0.4pt] (A) -- (Aend |- A);
  \draw[line width=0.4pt] (3.7,0) arc (0:15:0.55);
  \node[anchor=north west,inner sep=0.8pt,font=\scriptsize] at (3.72,0.02) {$15^\circ$};
  \foreach \tt in {0.12,0.28,0.44,0.60,0.76,0.92} {
    \pgfmathsetmacro{\hx}{3.15+\tt*2.2}
    \pgfmathsetmacro{\hy}{\tt*2.2*tan(15)}
    \pgfmathsetmacro{\hxx}{\hx-0.18}
    \pgfmathsetmacro{\hyy}{\hy-0.22}
    \draw[line width=0.35pt] ({\hx},{\hy}) -- ({\hxx},{\hyy});
  }
  \fill (O) circle (1.15pt);
  \fill (A) circle (1.1pt);
  \fill (P) circle (1.2pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.05,-0.05) {$O$};
  \node[anchor=north,inner sep=1.5pt] at (A) {$A$};
...
\centering
\fitfig{%
% fig-alt: 第10题 双曲线及其渐近线被一直线所截，交点依次为 P、R、S、Q，示 PR=SQ
\begin{tikzpicture}[x=0.55cm,y=0.55cm,line cap=round,line join=round,>=latex]
  \def\a{1.55}
  \def\b{1.25}
  \def\k{0.25}
  ...
  \draw[line width=0.7pt,-latex] (0,-3.4) -- (0,3.6) node[anchor=south,inner sep=2pt] {$y$};
  % asymptotes
  \draw[line width=0.5pt] ({-3.8},{-3.8*\b/\a}) -- ({3.8},{3.8*\b/\a});
  \draw[line width=0.5pt] ({-3.8},{3.8*\b/\a}) -- ({3.8},{-3.8*\b/\a});
  % hyperbola branches
  \draw[line width=0.85pt,domain=-1.55:1.55,samples=60,smooth]
    plot ({\a*cosh(\x)},{\b*sinh(\x)});
  \draw[line width=0.85pt,domain=-1.55:1.55,samples=60,smooth]
    plot ({-\a*cosh(\x)},{\b*sinh(\x)});
  % transversal y=kx+r
  \draw[line width=0.7pt] (-4.2,{\k*(-4.2)+\rr}) -- (4.2,{\k*4.2+\rr});
  % intersections with asymptotes y=±(b/a)x
  % kx+r = (b/a)x  => x = r / (b/a - k)
  \pgfmathsetmacro{\xs}{\rr / (\b/\a - \k)}
  \pgfmathsetmacro{\ys}{\k*\xs+\rr}
  % kx+r = -(b/a)x => x = r / (-b/a - k) wait: kx+r=-(b/a)x => x(k+b/a)=-r => x=-r/(k+b/a)
  \pgfmathsetmacro{\xr}{-\rr / (\k + \b/\a)}
  \pgfmathsetmacro{\yr}{\k*\xr+\rr}
  % intersections with hyperbola: substitute y=kx+r into b^2 x^2 - a^2 y^2 = a^2 b^2
  \pgfmathsetmacro{\Acoef}{\b*\b - \a*\a*\k*\k}
  \pgfmathsetmacro{\Bcoef}{-2*\a*\a*\k*\rr}
  \pgfmathsetmacro{\Ccoef}{-\a*\a*\rr*\rr - \a*\a*\b*\b}
  \pgfmathsetmacro{\disc}{sqrt(\Bcoef*\Bcoef-4*\Acoef*\Ccoef)}
  \pgfmathsetmacro{\xp}{(-\Bcoef-\disc)/(2*\Acoef)}
  \pgfmathsetmacro{\xq}{(-\Bcoef+\disc)/(2*\Acoef)}
  \pgfmathsetmacro{\yp}{\k*\xp+\rr}
  \pgfmathsetmacro{\yq}{\k*\xq+\rr}
...
\begin{tikzpicture}[x=0.85cm,y=0.85cm,line cap=round,line join=round,>=latex]
  ...
  \path[name intersections={of=bf and ec,by=P}];
  \fill (A) circle (1.1pt);
  \fill (B) circle (1.1pt);
  \fill (C) circle (1.1pt);
  \fill (D) circle (1.1pt);
...
\centering
\fitfig{%
% fig-alt: 图7·1 极坐标系：极点 O、极轴 Ox、点 P(ρ,θ)，极径 ρ 与极角 θ
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \coordinate (O) at (0,0);
  \coordinate (P) at (38:3.35);
  \draw[line width=0.75pt,-latex] (0,0) -- (4.15,0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.8pt] (O) -- (P);
  \draw[line width=0.5pt,-latex] (0.85,0) arc (0:38:0.85);
  \fill (O) circle (1.2pt);
  \fill (P) circle (1.35pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.05,-0.05) {$O$};
  \node[anchor=south,inner sep=1.5pt] at (19:1.85) {$\rho$};
  \node[anchor=west,inner sep=1pt,font=\small] at (0.95,0.32) {$\theta$};
  \node[anchor=west,inner sep=2pt] at (P) {$P(\rho,\theta)$};
\end{tikzpicture}%
}
\caption{图7·1}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·2 极坐标网格上描点 A(3,π/6)、B(4,225°)、C(8,-π/3)、D(5,π)
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\u{0.26}
  \pgfmathsetmacro{\xmax}{\u*10+0.42}
  \pgfmathsetmacro{\rout}{\u*10+0.26}
  \foreach \r in {1,...,10} {
    \draw[line width=0.28pt] (0,0) circle ({\r*\u});
  }
  \foreach \ang in {0,30,...,330} {
    \draw[line width=0.28pt] (0,0) -- ({\ang}:{\u*10});
  }
  \foreach \ang in {15,45,...,345} {
    \draw[dash pattern=on 1.0pt off 0.85pt,line width=0.2pt] (0,0) -- ({\ang}:{\u*10});
  }
  % extra ray for 225°
  \draw[dash pattern=on 1.0pt off 0.85pt,line width=0.28pt] (0,0) -- ({225}:{\u*10});
  \draw[line width=0.7pt,-latex] (0,0) -- ({\xmax},0) node[anchor=west,inner sep=1.5pt] {$x$};
  \fill (0,0) circle (1.05pt);
  \node[anchor=north east,inner sep=1pt,font=\small] at (-0.05,-0.05) {$O$};
  \node[anchor=north,inner sep=0.6pt,font=\footnotesize] at ({5*\u},-0.02) {$5$};
  \node[anchor=north,inner sep=0.6pt,font=\footnotesize] at ({10*\u},-0.02) {$10$};
  \foreach \ang/\lab in {30/30^\circ,60/60^\circ,90/90^\circ,120/120^\circ,150/150^\circ,180/-180^\circ,210/210^\circ,240/240^\circ,270/270^\circ,300/300^\circ,330/330^\circ} {
    \node[font=\scriptsize,inner sep=0.5pt] at ({\ang}:{\rout}) {$\lab$};
  }
  % points
  \coordinate (A) at ({30}:{\u*3});
  \coordinate (B) at ({225}:{\u*4});
  \coordinate (C) at ({300}:{\u*8});
  \coordinate (D) at ({180}:{\u*5});
  \draw[line width=0.75pt,-latex] (0,0) -- (A);
  \draw[line width=0.75pt,-latex] (0,0) -- (B);
  \draw[line width=0.75pt,-latex] (0,0) -- (C);
  \draw[line width=0.75pt,-latex] (0,0) -- (D);
  \fill (A) circle (1.2pt);
  \fill (B) circle (1.2pt);
  \fill (C) circle (1.2pt);
  \fill (D) circle (1.2pt);
  \node[anchor=south west,inner sep=1pt,font=\small] at (A) {$A\!\left(3,\dfrac{\pi}{6}\right)$};
  \node[anchor=north east,inner sep=1pt,font=\small] at (B) {$B(4,225^\circ)$};
  \node[anchor=north west,inner sep=1pt,font=\small] at (C) {$C\!\left(8,-\dfrac{\pi}{3}\right)$};
  \node[anchor=south east,inner sep=1pt,font=\small] at (D) {$D(5,\pi)$};
\end{tikzpicture}%
}
\caption{图7·2}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·3 ρ<0 时点 P(ρ,θ) 在射线 OM 的反向延长线上，|OP|=|ρ|
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\pang{52}
  \pgfmathsetmacro{\popp}{\pang+180}
  \coordinate (O) at (0,0);
  \coordinate (M) at ({\pang}:2.85);
  \coordinate (P) at ({\popp}:2.35);
  \draw[line width=0.75pt,-latex] (0,0) -- (3.55,0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.75pt] (P) -- (M);
  \draw[line width=0.5pt,-latex] (0.78,0) arc (0:\pang:0.78);
  \fill (O) circle (1.2pt);
  \fill (P) circle (1.35pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.05,-0.05) {$O$};
  \node[anchor=south west,inner sep=1.5pt] at (M) {$M$};
  \node[anchor=west,inner sep=1pt,font=\small] at (0.88,0.38) {$\theta$};
  \node[anchor=north,inner sep=1.2pt,font=\small] at ({\popp}:1.15) {$|\rho|$};
  \node[anchor=east,inner sep=2pt] at (P) {$P(\rho,\theta)$};
  \node[anchor=east,inner sep=2pt,yshift=-11pt,font=\small] at (P) {$\rho<0$};
\end{tikzpicture}%
}
\caption{图7·3}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·4 极坐标网格上描负极径点 A(-4,π/3)、B(-6,3π/2)、C(-9,-5π/4)、D(-5,210°)，并标出 A₁
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\u{0.26}
  \pgfmathsetmacro{\xmax}{\u*10+0.42}
  \pgfmathsetmacro{\rout}{\u*10+0.26}
  ...
  % A(-4, π/3) -> plot at 240°, A1 at 60° r=4
  \coordinate (A) at ({240}:{\u*4});
  \coordinate (Aone) at ({60}:{\u*4});
  % B(-6, 3π/2) -> 90°
  \coordinate (B) at ({90}:{\u*6});
  % C(-9, -5π/4) -> 315°
  \coordinate (C) at ({315}:{\u*9});
  % D(-5, 210°) -> 30°
  \coordinate (D) at ({30}:{\u*5});
  \draw[line width=0.7pt,-latex] (0,0) -- (Aone);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.55pt] (Aone) -- (A);
  \draw[line width=0.75pt,-latex] (0,0) -- (B);
  \draw[line width=0.75pt,-latex] (0,0) -- (C);
  \draw[line width=0.75pt,-latex] (0,0) -- (D);
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.45pt] (0,0) -- ({210}:{\u*5.5});
  \fill (A) circle (1.2pt);
  \fill (Aone) circle (1.1pt);
  \fill (B) circle (1.2pt);
  \fill (C) circle (1.2pt);
  \fill (D) circle (1.2pt);
  \node[anchor=north east,inner sep=1pt,font=\small] at (A) {$A\!\left(-4,\dfrac{\pi}{3}\right)$};
  \node[anchor=south west,inner sep=1pt,font=\small] at (Aone) {$A_1$};
  \node[anchor=south east,inner sep=1pt,font=\small] at (B) {$B\!\left(-6,\dfrac{3\pi}{2}\right)$};
  \node[anchor=west,inner sep=1pt,font=\small] at (C) {$C\!\left(-9,-\dfrac{5\pi}{4}\right)$};
  \node[anchor=south west,inner sep=1pt,font=\small] at (D) {$D(-5,210^\circ)$};
\end{tikzpicture}%
}
\caption{图7·4}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·6 极坐标网格上点 A 位于 (6,5π/3) 即 (6,300°)，过原点的直径示正负极径同一点
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\u{0.26}
  \pgfmathsetmacro{\xmax}{\u*10+0.42}
  \pgfmathsetmacro{\rout}{\u*10+0.26}
  ...
  \coordinate (A) at ({300}:{\u*6});
  \coordinate (Aopp) at ({120}:{\u*6});
  \draw[line width=0.85pt] (Aopp) -- (A);
  \draw[line width=0.75pt,-latex] (0,0) -- (A);
  \fill (A) circle (1.35pt);
...
\centering
\fitfig{%
% fig-alt: 图7·7 极坐标网格上 P₁(5,π/6)、P₂(5,-π/6)、P₃(-5,-π/6)、P₄(-5,π/6)，示关于极轴、极垂线、极点的对称
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\u{0.26}
  \pgfmathsetmacro{\xmax}{\u*10+0.42}
  \pgfmathsetmacro{\rout}{\u*10+0.26}
  ...
  \coordinate (P1) at ({30}:{\u*5});
  \coordinate (P2) at ({330}:{\u*5});
  \coordinate (P3) at ({150}:{\u*5});
  \coordinate (P4) at ({210}:{\u*5});
  \draw[line width=0.8pt] (P1) -- (P4);
  \draw[line width=0.8pt] (P2) -- (P3);
  \draw[line width=0.75pt,-latex] (0,0) -- (P1);
  \draw[line width=0.75pt,-latex] (0,0) -- (P2);
  \draw[line width=0.75pt,-latex] (0,0) -- (P3);
  \draw[line width=0.75pt,-latex] (0,0) -- (P4);
  \fill (P1) circle (1.3pt);
  \fill (P2) circle (1.3pt);
  \fill (P3) circle (1.3pt);
  \fill (P4) circle (1.3pt);
  \node[anchor=south west,inner sep=1.2pt,font=\small] at (P1) {$P_1$};
  \node[anchor=north west,inner sep=1.2pt,font=\small] at (P2) {$P_2$};
  \node[anchor=south east,inner sep=1.2pt,font=\small] at (P3) {$P_3$};
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (P4) {$P_4$};
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: 第1题 极坐标网格上描点 A(4,120°)、B(8,330°)、C(5,0°)、D(8,210°)、E(6,45°)、F(7,90°)
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\u{0.26}
  \pgfmathsetmacro{\xmax}{\u*10+0.42}
  \pgfmathsetmacro{\rout}{\u*10+0.26}
  ...
  \coordinate (A) at ({120}:{\u*4});
  \coordinate (B) at ({330}:{\u*8});
  \coordinate (C) at ({0}:{\u*5});
  \coordinate (D) at ({210}:{\u*8});
  \coordinate (E) at ({45}:{\u*6});
  \coordinate (F) at ({90}:{\u*7});
  \fill (A) circle (1.35pt);
  \fill (B) circle (1.35pt);
  \fill (C) circle (1.35pt);
  \fill (D) circle (1.35pt);
  \fill (E) circle (1.35pt);
  \fill (F) circle (1.35pt);
  \node[anchor=south east,inner sep=1.2pt,font=\small] at (A) {$A$};
  \node[anchor=north,inner sep=1.5pt,font=\small] at (B) {$B$};
  \node[anchor=south,inner sep=1.5pt,font=\small] at (C) {$C$};
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (D) {$D$};
  \node[anchor=south east,inner sep=1.2pt,font=\small] at (E) {$E$};
  \node[anchor=east,inner sep=1.5pt,font=\small] at (F) {$F$};
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: 图7·8 直角坐标与极坐标互化：点 P(x,y)=P(ρ,θ)，OM=x，MP=y，OP=ρ，∠xOP=θ
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \coordinate (O) at (0,0);
  \coordinate (M) at (2.55,0);
  \coordinate (P) at (2.55,1.85);
  \draw[line width=0.7pt,-latex] (-0.45,0) -- (3.55,0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.7pt,-latex] (0,-0.35) -- (0,2.85) node[anchor=south,inner sep=2pt] {$y$};
  \draw[line width=0.85pt] (O) -- (P);
  \draw[line width=0.55pt] (P) -- (M);
  \draw[line width=0.4pt] ({2.55-0.18},0) -- ({2.55-0.18},0.18) -- (2.55,0.18);
  \draw[line width=0.45pt,-latex] (0.72,0) arc (0:{atan(1.85/2.55)}:0.72);
  \fill (O) circle (1.15pt);
  \fill (M) circle (1.05pt);
  \fill (P) circle (1.25pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.05,-0.05) {$O$};
  \node[anchor=north,inner sep=1.5pt] at (M) {$M$};
  \node[anchor=south west,inner sep=1.5pt,align=left] at (P) {$P(x,y)$\\$(\rho,\theta)$};
  \node[anchor=north,inner sep=1pt,font=\small] at (1.25,0) {$x$};
  \node[anchor=west,inner sep=1.5pt,font=\small] at (2.55,0.95) {$y$};
  \node[anchor=south east,inner sep=1pt,font=\small] at (1.15,1.05) {$\rho$};
  \node[anchor=west,inner sep=1pt,font=\small] at (0.78,0.28) {$\theta$};
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: 图7·9 极坐标系中的心脏线 ρ=a(1+cosθ)（a=5），极轴方向外端达 ρ=10，极点处成尖点
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\u{0.26}
  \pgfmathsetmacro{\xmax}{\u*10+0.42}
  \pgfmathsetmacro{\rout}{\u*10+0.26}
  ...
  \draw[line width=0.95pt,domain=0:360,samples=180,smooth]
    plot ({\x}:{(\u*5)*(1+cos(\x))});
  \foreach \ang in {0,30,60,90,120,240,270,300,330} {
    \pgfmathsetmacro{\rr}{(\u*5)*(1+cos(\ang))}
    \fill ({\ang}:{\rr}) circle (1.05pt);
  }
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: 图7·10 极坐标系中的双纽线 ρ²=a²cos2θ（a=10），左右两叶，极轴两端标 a
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\u{0.26}
  \pgfmathsetmacro{\xmax}{\u*10+0.42}
  \pgfmathsetmacro{\rout}{\u*10+0.26}
  ...
  \node[anchor=north east,inner sep=1pt,font=\small] at (-0.05,-0.05) {$O$};
  \node[anchor=north,inner sep=0.6pt,font=\footnotesize] at ({10*\u},-0.02) {$10$};
  \foreach \ang/\lab in {30/30^\circ,45/45^\circ,60/60^\circ,90/90^\circ,120/120^\circ,135/135^\circ,150/150^\circ,180/180^\circ,210/210^\circ,225/225^\circ,240/240^\circ,270/270^\circ,300/300^\circ,315/315^\circ,330/330^\circ} {
    \node[font=\scriptsize,inner sep=0.5pt] at ({\ang}:{\rout}) {$\lab$};
  }
  \draw[line width=0.95pt,domain=-45:45,samples=80,smooth]
    plot ({\x}:{\u*10*sqrt(cos(2*\x))});
  \draw[line width=0.95pt,domain=135:225,samples=80,smooth]
    plot ({\x}:{\u*10*sqrt(cos(2*\x))});
  \node[anchor=south,inner sep=1pt,font=\small] at ({0}:{\u*5}) {$a$};
  \node[anchor=south,inner sep=1pt,font=\small] at ({180}:{\u*5}) {$a$};
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: 图7·11 极坐标系中的抛物线 ρ=2/(1-sinθ)，开口向上，顶点在 (1,270°)，过 (2,0°) 与 (2,180°)
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\u{0.26}
  \pgfmathsetmacro{\xmax}{\u*10+0.42}
  \pgfmathsetmacro{\rout}{\u*10+0.26}
  ...
  \begin{scope}
    \clip (0,0) circle ({\u*10+0.02});
    \draw[line width=0.95pt,domain=-80:80,samples=120,smooth]
      plot ({\x}:{\u*2/(1-sin(\x))});
    \draw[line width=0.95pt,domain=100:260,samples=120,smooth]
      plot ({\x}:{\u*2/(1-sin(\x))});
  \end{scope}
  \foreach \ang/\rr in {-90/1,-60/1.1,-45/1.2,-30/1.3,0/2,30/4,45/6.8} {
    \fill ({\ang}:{\u*\rr}) circle (1.0pt);
  }
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: 图7·12 过 A(a,0) 且垂直于极轴的直线 l；点 P(ρ,θ)，直角三角形 OAP，ρ cosθ=a
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \coordinate (O) at (0,0);
  \coordinate (A) at (2.6,0);
  \coordinate (P) at (2.6,1.85);
  \draw[dash pattern=on 2.2pt off 1.4pt,line width=0.65pt,-latex]
    (-0.35,0) -- (4.15,0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.85pt] (2.6,-1.55) -- (2.6,2.55) node[anchor=south,inner sep=2pt] {$l$};
  \draw[line width=0.8pt] (O) -- (P);
  \draw[line width=0.4pt] ({2.6-0.18},0) -- ({2.6-0.18},0.18) -- (2.6,0.18);
  \draw[line width=0.45pt,-latex] (0.7,0) arc (0:{atan(1.85/2.6)}:0.7);
  \fill (O) circle (1.15pt);
  \fill (A) circle (1.1pt);
  \fill (P) circle (1.2pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.05,-0.05) {$O$};
  \node[anchor=north west,inner sep=1.5pt] at (A) {$A(a,0)$};
  \node[anchor=west,inner sep=1.5pt] at (P) {$P(\rho,\theta)$};
  \node[anchor=south east,inner sep=1pt,font=\small] at (1.2,1.05) {$\rho$};
  \node[anchor=west,inner sep=1pt,font=\small] at (0.78,0.28) {$\theta$};
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: 图7·13 以 OA 为直径且过极点 O 的圆；直角顶点 P(ρ,θ) 的轨迹，ρ=2r cosθ
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\rr{1.35}
  \coordinate (O) at (0,0);
  \coordinate (A) at ({2*\rr},0);
  \coordinate (C) at ({\rr},0);
  \coordinate (P) at ({2*\rr*cos(38)},{2*\rr*sin(38)});
  \draw[dash pattern=on 2.2pt off 1.4pt,line width=0.65pt,-latex]
    (-0.35,0) -- ({2*\rr+1.15},0) node[anchor=west,inner sep=2pt] {$x$};
  \draw[line width=0.85pt] (C) circle ({\rr});
  \draw[line width=0.8pt] (O) -- (P);
  \draw[line width=0.7pt] (P) -- (A);
  \draw[line width=0.45pt] (0.55,0) arc (0:38:0.55);
  \fill (O) circle (1.15pt);
  \fill (A) circle (1.1pt);
  \fill (P) circle (1.2pt);
  \node[anchor=north east,inner sep=1.5pt] at (-0.05,-0.05) {$O$};
  \node[anchor=north,inner sep=1.5pt] at (A) {$A$};
  \node[anchor=south west,inner sep=1.5pt] at (P) {$P(\rho,\theta)$};
  \node[anchor=south east,inner sep=0.8pt,font=\small] at ({0.95*\rr*cos(38)},{0.95*\rr*sin(38)}) {$\rho$};
  \node[anchor=west,inner sep=1pt,font=\small] at (0.62,0.22) {$\theta$};
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: 图7·14 圆 ρ=a cosθ 的弦 OQ 延长至 P 使 QP=a；Q(ρ₁,θ₁)、P(ρ,θ)，轨迹为心脏线 ρ=a(1+cosθ)
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{1.42}
  \def\th{52}
  \coordinate (O) at (0,0);
  \coordinate (C) at ({\aa/2},0);
  \coordinate (Q) at ({\th}:{\aa*cos(\th)});
  \coordinate (P) at ({\th}:{\aa*(1+cos(\th))});
  \draw[line width=0.65pt,-latex] (-0.42,0) -- ({\aa*2+0.72},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.82pt] (C) circle ({\aa/2});
  \draw[line width=0.9pt,domain=0:360,samples=160,smooth]
    plot ({\aa*(1+cos(\x))*cos(\x)}, {\aa*(1+cos(\x))*sin(\x)});
  \draw[line width=0.75pt] (O) -- (P);
  \draw[line width=0.45pt] (0.42,0) arc (0:\th:0.42);
  \fill (O) circle (1.15pt);
  \fill (Q) circle (1.15pt);
  \fill (P) circle (1.2pt);
  \node[anchor=east,inner sep=2pt,font=\small] at (O) {$O$};
  \node[anchor=south west,inner sep=1.2pt,font=\small] at (Q) {$Q(\rho_1,\theta_1)$};
  \node[anchor=south,inner sep=2pt,font=\small] at (P) {$P(\rho,\theta)$};
  \node[anchor=west,inner sep=1pt,font=\small] at (0.46,0.16) {$\theta$};
\end{tikzpicture}%
}
\caption{图7·14}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 蜗线 ρ=a cosθ+b（a＞b）有内环；圆 ρ=a cosθ，射线过 P′、Q、P
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{1.48}
  \def\bb{0.74}
  \def\th{24}
  \pgfmathsetmacro{\rP}{\bb+\aa*cos(\th)}
  \pgfmathsetmacro{\rQ}{\aa*cos(\th)}
  \pgfmathsetmacro{\rPp}{abs(\bb+\aa*cos(\th+180))}
  \coordinate (O) at (0,0);
  \coordinate (A) at ({\aa},0);
  \coordinate (C) at ({\aa/2},0);
  \coordinate (P) at ({\th}:{\rP});
  \coordinate (Q) at ({\th}:{\rQ});
  \coordinate (Pp) at ({\th}:{\rPp});
  \draw[line width=0.55pt] (0,{-(\aa*0.95)}) -- (0,{\aa*1.15});
  \draw[line width=0.55pt] ({\th+180}:{\aa*0.95}) -- ({\th}:{\rP+0.55});
  \draw[line width=0.65pt,-latex] (-{\aa*0.55},0) -- ({\aa+\bb+0.85},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.82pt] (C) circle ({\aa/2});
  \draw[line width=0.9pt,domain=0:360,samples=200,smooth]
    plot ({(\bb+\aa*cos(\x))*cos(\x)}, {(\bb+\aa*cos(\x))*sin(\x)});
  \fill (O) circle (1.1pt);
  \fill (A) circle (1.05pt);
  \fill (Q) circle (1.1pt);
  \fill (P) circle (1.15pt);
  \fill (Pp) circle (1.05pt);
  \node[anchor=north east,inner sep=1.4pt,font=\small] at (O) {$O$};
  \node[anchor=north,inner sep=1.6pt,font=\small] at (A) {$A$};
  \node[anchor=south,inner sep=1.4pt,font=\small] at (Q) {$Q$};
  \node[anchor=south west,inner sep=1.4pt,font=\small] at (P) {$P$};
  \node[anchor=south,inner sep=1.2pt,font=\small] at (Pp) {$P'$};
  \node[anchor=west,inner sep=0.8pt,font=\scriptsize] at (0.42,0.10) {$\theta$};
  \node[anchor=north,inner sep=4pt,font=\small] at (0,{-(\aa+\bb)*0.42}) {$a>b$};
\end{tikzpicture}%
}
\caption{蜗线（a＞b）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 蜗线 ρ=a cosθ+b（a＝b）即心脏线；圆过极点，尖点在 O，标 P′
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{1.32}
  \def\th{68}
  \coordinate (O) at (0,0);
  ...
  \coordinate (Q) at ({\th}:{\aa*cos(\th)});
  \coordinate (P) at ({\th}:{\aa*(1+cos(\th))});
  \draw[line width=0.65pt,-latex] (-0.72,0) -- ({\aa*2+0.7},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.82pt] (C) circle ({\aa/2});
  \draw[line width=0.9pt,domain=0:360,samples=160,smooth]
    plot ({\aa*(1+cos(\x))*cos(\x)}, {\aa*(1+cos(\x))*sin(\x)});
  \draw[line width=0.75pt] (O) -- (P);
  \draw[line width=0.45pt] (0.38,0) arc (0:\th:0.38);
  \fill (O) circle (1.1pt);
  \fill (A) circle (1.05pt);
  \fill (Q) circle (1.1pt);
  \fill (P) circle (1.15pt);
  \node[anchor=south east,inner sep=1.5pt,font=\small] at (O) {$O$};
  \node[anchor=north,inner sep=1.6pt,font=\small] at (A) {$A$};
  \node[anchor=south east,inner sep=1.3pt,font=\small] at (Q) {$Q$};
  \node[anchor=south,inner sep=2pt,font=\small] at (P) {$P$};
  \node[anchor=east,inner sep=2.4pt,font=\small] at (-0.12,0) {$P'$};
  \node[anchor=west,inner sep=1pt,font=\small] at (0.42,0.18) {$\theta$};
  \node[anchor=north,inner sep=5pt,font=\small] at ({\aa*0.15},{-(\aa)*0.55}) {$a=b$};
\end{tikzpicture}%
}
\caption{蜗线（a＝b）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·15 蜗线 ρ=a cosθ+b（a＜b）无内环，左侧微凹；圆过极点，射线 OQP
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{1.18}
  \def\bb{1.95}
  \def\th{56}
  ...
  \coordinate (C) at ({\aa/2},0);
  \coordinate (Q) at ({\th}:{\aa*cos(\th)});
  \coordinate (P) at ({\th}:{\bb+\aa*cos(\th)});
  \coordinate (Pp) at ({180}:{\bb-\aa});
  \draw[line width=0.65pt,-latex] ({-(\bb-\aa)-0.62},0) -- ({\aa+\bb+0.7},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.82pt] (C) circle ({\aa/2});
  \draw[line width=0.9pt,domain=0:360,samples=180,smooth]
    plot ({(\bb+\aa*cos(\x))*cos(\x)}, {(\bb+\aa*cos(\x))*sin(\x)});
  \draw[line width=0.75pt] (O) -- (P);
  \draw[line width=0.45pt] (0.36,0) arc (0:\th:0.36);
  \fill (O) circle (1.1pt);
  \fill (A) circle (1.05pt);
  \fill (Q) circle (1.1pt);
  \fill (P) circle (1.15pt);
  \fill (Pp) circle (1.05pt);
  \node[anchor=north,inner sep=1.8pt,font=\small] at (0.08,-0.06) {$O$};
  \node[anchor=north,inner sep=1.6pt,font=\small] at (A) {$A$};
  \node[anchor=south east,inner sep=1.2pt,font=\small] at (Q) {$Q$};
  \node[anchor=south west,inner sep=1.3pt,font=\small] at (P) {$P$};
  \node[anchor=east,inner sep=2pt,font=\small] at (Pp) {$P'$};
  \node[anchor=west,inner sep=1pt,font=\small] at (0.40,0.16) {$\theta$};
  \node[anchor=north,inner sep=5pt,font=\small] at ({\aa*0.2},{-(\aa)*0.72}) {$a<b$};
\end{tikzpicture}%
}
\caption{图7·15}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 第1（4）题 三叶玫瑰线 ρ=a cos3θ（a＞0），叶在 0°、120°、240°，各叶标 a
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{2.05}
  \def\rl{2.48}
  \def\rout{2.72}
  \foreach \ang in {30,60,90,120,150,180,210,240,270,300,330} {
    \draw[line width=0.3pt] (0,0) -- ({\ang}:{\rl});
  }
  \draw[line width=0.7pt,-latex] (0,0) -- ({\rl+0.42},0) node[anchor=west,inner sep=1.5pt] {$x$};
  \fill (0,0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (-0.04,-0.04) {$O$};
  \foreach \ang/\lab in {30/30^\circ,60/60^\circ,90/90^\circ,120/120^\circ,150/150^\circ,180/180^\circ,210/210^\circ,240/240^\circ,270/270^\circ,300/300^\circ,330/330^\circ} {
    \node[font=\scriptsize,inner sep=0.5pt] at ({\ang}:{\rout}) {$\lab$};
  }
  \draw[line width=0.95pt,domain=-30:30,samples=50,smooth]
    plot ({\aa*cos(3*\x)*cos(\x)}, {\aa*cos(3*\x)*sin(\x)});
  \draw[line width=0.95pt,domain=90:150,samples=50,smooth]
    plot ({\aa*cos(3*\x)*cos(\x)}, {\aa*cos(3*\x)*sin(\x)});
  \draw[line width=0.95pt,domain=210:270,samples=50,smooth]
    plot ({\aa*cos(3*\x)*cos(\x)}, {\aa*cos(3*\x)*sin(\x)});
  \node[anchor=south,inner sep=1pt,font=\small] at ({0}:{\aa*0.50}) {$a$};
  \node[anchor=south east,inner sep=1pt,font=\small] at ({120}:{\aa*0.50}) {$a$};
  \node[anchor=north east,inner sep=1pt,font=\small] at ({240}:{\aa*0.50}) {$a$};
\end{tikzpicture}%
}
\caption{（第1（4）题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 第1（5）题 三叶玫瑰线 ρ=a sin3θ（a＞0），叶在 30°、150°、270°，各叶标 a
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{2.05}
  \def\rl{2.48}
  \def\rout{2.72}
  ...
  \draw[line width=0.55pt] (0,0) -- ({180}:{\rl});
  \draw[line width=0.7pt,-latex] (0,0) -- ({\rl+0.42},0) node[anchor=west,inner sep=1.5pt] {$x$};
  \node[anchor=east,inner sep=1.5pt,font=\small] at ({180}:{\rl}) {$x$};
  \fill (0,0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (-0.04,-0.04) {$O$};
  \foreach \ang/\lab in {30/30^\circ,90/90^\circ,150/150^\circ,180/180^\circ,210/210^\circ,270/270^\circ,330/330^\circ} {
    \node[font=\scriptsize,inner sep=0.5pt] at ({\ang}:{\rout}) {$\lab$};
  }
  \draw[line width=0.95pt,domain=0:60,samples=50,smooth]
    plot ({\aa*sin(3*\x)*cos(\x)}, {\aa*sin(3*\x)*sin(\x)});
  \draw[line width=0.95pt,domain=120:180,samples=50,smooth]
    plot ({\aa*sin(3*\x)*cos(\x)}, {\aa*sin(3*\x)*sin(\x)});
  \draw[line width=0.95pt,domain=240:300,samples=50,smooth]
    plot ({\aa*sin(3*\x)*cos(\x)}, {\aa*sin(3*\x)*sin(\x)});
  \node[anchor=south east,inner sep=1pt,font=\small] at ({30}:{\aa*0.50}) {$a$};
  \node[anchor=south west,inner sep=1pt,font=\small] at ({150}:{\aa*0.50}) {$a$};
  \node[anchor=east,inner sep=1.4pt,font=\small] at ({270}:{\aa*0.50}) {$a$};
\end{tikzpicture}%
}
\caption{（第1（5）题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 第1（6）题 四叶玫瑰线 ρ=a sin2θ（a＞0），叶在 45°、135°、225°、315°，各叶标 a
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{2.00}
  \def\rl{2.42}
  \def\rout{2.66}
  \foreach \ang in {45,135,225,315} {
    \draw[dash pattern=on 1.15pt off 0.9pt,line width=0.28pt] (0,0) -- ({\ang}:{\rl});
  }
  \draw[line width=0.55pt] (0,{-(\rl)}) -- (0,{\rl});
  \draw[line width=0.55pt] ({-(\rl)},0) -- (0,0);
  \draw[line width=0.7pt,-latex] (0,0) -- ({\rl+0.42},0) node[anchor=west,inner sep=1.5pt] {$x$};
  \fill (0,0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (-0.04,-0.04) {$O$};
  \foreach \ang/\lab in {45/45^\circ,90/90^\circ,135/135^\circ,225/225^\circ,315/315^\circ} {
    \node[font=\scriptsize,inner sep=0.5pt] at ({\ang}:{\rout}) {$\lab$};
  }
  \draw[line width=0.95pt,domain=0:90,samples=50,smooth]
    plot ({\aa*sin(2*\x)*cos(\x)}, {\aa*sin(2*\x)*sin(\x)});
  \draw[line width=0.95pt,domain=90:180,samples=50,smooth]
    plot ({\aa*sin(2*\x)*cos(\x)}, {\aa*sin(2*\x)*sin(\x)});
  \draw[line width=0.95pt,domain=180:270,samples=50,smooth]
    plot ({\aa*sin(2*\x)*cos(\x)}, {\aa*sin(2*\x)*sin(\x)});
  \draw[line width=0.95pt,domain=270:360,samples=50,smooth]
    plot ({\aa*sin(2*\x)*cos(\x)}, {\aa*sin(2*\x)*sin(\x)});
  \node[anchor=south east,inner sep=0.8pt,font=\small] at ({45}:{\aa*0.48}) {$a$};
  \node[anchor=south west,inner sep=0.8pt,font=\small] at ({135}:{\aa*0.48}) {$a$};
  \node[anchor=north west,inner sep=0.8pt,font=\small] at ({225}:{\aa*0.48}) {$a$};
  \node[anchor=north east,inner sep=0.8pt,font=\small] at ({315}:{\aa*0.48}) {$a$};
\end{tikzpicture}%
}
\caption{（第1（6）题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 第1（7）题 四叶玫瑰线 ρ=a cos2θ（a＞0），叶在 0°、90°、180°、270°，各叶标 a
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{2.00}
  \def\rl{2.42}
  \def\rout{2.66}
  ...
  \draw[line width=0.95pt,domain=-45:45,samples=50,smooth]
    plot ({\aa*cos(2*\x)*cos(\x)}, {\aa*cos(2*\x)*sin(\x)});
  \draw[line width=0.95pt,domain=45:135,samples=50,smooth]
    plot ({\aa*cos(2*\x)*cos(\x)}, {\aa*cos(2*\x)*sin(\x)});
  \draw[line width=0.95pt,domain=135:225,samples=50,smooth]
    plot ({\aa*cos(2*\x)*cos(\x)}, {\aa*cos(2*\x)*sin(\x)});
  \draw[line width=0.95pt,domain=225:315,samples=50,smooth]
    plot ({\aa*cos(2*\x)*cos(\x)}, {\aa*cos(2*\x)*sin(\x)});
  \node[anchor=south,inner sep=1pt,font=\small] at ({0}:{\aa*0.50}) {$a$};
  \node[anchor=east,inner sep=1.4pt,font=\small] at ({90}:{\aa*0.50}) {$a$};
  \node[anchor=south,inner sep=1pt,font=\small] at ({180}:{\aa*0.50}) {$a$};
  \node[anchor=east,inner sep=1.4pt,font=\small] at ({270}:{\aa*0.50}) {$a$};
\end{tikzpicture}%
}
\caption{（第1（7）题）}
...
\centering
\fitfig{%
% fig-alt: 图7·16 直线极坐标方程：极点 O 到直线 l 垂足 D，OD=p，∠xOD=ω；直线上点 P(ρ,θ)，OA=a
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\pp{2.05}
  \def\ww{52}
  \def\th{22}
  \pgfmathsetmacro{\rhop}{\pp/cos(\ww-\th)}
  \coordinate (O) at (0,0);
  \coordinate (D) at ({\ww}:{\pp});
  \coordinate (P) at ({\th}:{\rhop});
  \pgfmathsetmacro{\aa}{\pp/cos(\ww)}
  \coordinate (A) at ({\aa},0);
  \pgfmathsetmacro{\tx}{cos(\ww-90)}
  \pgfmathsetmacro{\ty}{sin(\ww-90)}
  \coordinate (L1) at ({(\pp)*cos(\ww)-1.55*\tx},{(\pp)*sin(\ww)-1.55*\ty});
  \coordinate (L2) at ({(\pp)*cos(\ww)+2.45*\tx},{(\pp)*sin(\ww)+2.45*\ty});
  \draw[line width=0.65pt,-latex] (-0.35,0) -- ({\aa+1.15},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.85pt] (L1) -- (L2);
  \draw[line width=0.75pt] (O) -- (D);
  \draw[line width=0.75pt] (O) -- (P);
  \pgfmathsetmacro{\s}{0.18}
  % right angle inside triangle ODP: back along OD and toward A (ω-90)
  \draw[line width=0.4pt]
    ({(\pp)*cos(\ww)+\s*cos(\ww+180)},{(\pp)*sin(\ww)+\s*sin(\ww+180)})
    -- ({(\pp)*cos(\ww)+\s*cos(\ww+180)+\s*cos(\ww-90)},{(\pp)*sin(\ww)+\s*sin(\ww+180)+\s*sin(\ww-90)})
    -- ({(\pp)*cos(\ww)+\s*cos(\ww-90)},{(\pp)*sin(\ww)+\s*sin(\ww-90)});
  \draw[line width=0.4pt] (0.55,0) arc (0:\th:0.55);
  \draw[line width=0.4pt] (0.85,0) arc (0:\ww:0.85);
  \fill (O) circle (1.15pt);
  \fill (D) circle (1.1pt);
  \fill (P) circle (1.15pt);
  \fill (A) circle (1.05pt);
  \node[anchor=east,inner sep=2pt,font=\small] at (O) {$O$};
  \node[anchor=south west,inner sep=1.2pt,font=\small] at (D) {$D$};
  \node[anchor=west,inner sep=1.5pt,font=\small] at (P) {$P(\rho,\theta)$};
  \node[anchor=north,inner sep=1.5pt,font=\small] at (A) {$A$};
  \node[anchor=north west,inner sep=1pt,font=\small] at (L2) {$l$};
  \node[anchor=south east,inner sep=1pt,font=\small] at ({0.52*\pp*cos(\ww)},{0.52*\pp*sin(\ww)}) {$p$};
  \node[anchor=north west,inner sep=0.8pt,font=\small] at ({0.55*\rhop*cos(\th)},{0.55*\rhop*sin(\th)}) {$\rho$};
  \node[anchor=west,inner sep=0.6pt,font=\scriptsize] at ({0.58*cos(\th/2)},{0.58*sin(\th/2)}) {$\theta$};
  \node[anchor=west,inner sep=0.6pt,font=\scriptsize] at ({0.92*cos((\th+\ww)/2)},{0.92*sin((\th+\ww)/2)}) {$\omega$};
  \node[anchor=north,inner sep=2pt,font=\small] at ({\aa*0.48},0) {$a$};
\end{tikzpicture}%
}
\caption{图7·16}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·17（a）一般圆：圆心 C(ρ₁,θ₁)，圆上点 P(ρ,θ)，半径 r；虚线 OC、CP，实线 OP
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\rone{1.85}
  \def\tone{38}
  \def\rr{1.05}
  \coordinate (O) at (0,0);
  \coordinate (C) at ({\tone}:{\rone});
  \coordinate (P) at ({(\rone)*cos(\tone)+\rr*cos(105)},{(\rone)*sin(\tone)+\rr*sin(105)});
  \pgfmathsetmacro{\thP}{atan2({(\rone)*sin(\tone)+\rr*sin(105)},{(\rone)*cos(\tone)+\rr*cos(105)})}
  \draw[line width=0.65pt,-latex] (-0.35,0) -- ({\rone+\rr+0.85},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.85pt] (C) circle ({\rr});
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.55pt] (O) -- (C);
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.55pt] (C) -- (P);
  \draw[line width=0.75pt] (O) -- (P);
  \draw[line width=0.4pt] (0.48,0) arc (0:\tone:0.48);
  \draw[line width=0.4pt] (0.78,0) arc (0:\thP:0.78);
  \fill (O) circle (1.1pt);
  \fill (C) circle (1.15pt);
  \fill (P) circle (1.15pt);
  \node[anchor=north east,inner sep=1.4pt,font=\small] at (O) {$O$};
  \node[anchor=south west,inner sep=1.2pt,font=\small] at (C) {$C(\rho_1,\theta_1)$};
  \node[anchor=south,inner sep=1.5pt,font=\small] at (P) {$P(\rho,\theta)$};
  \node[anchor=west,inner sep=1pt,font=\small] at ({(\rone)*cos(\tone)+0.5*\rr*cos(105)},{(\rone)*sin(\tone)+0.5*\rr*sin(105)}) {$r$};
  \node[anchor=west,inner sep=0.5pt,font=\scriptsize] at ({0.52*cos(\tone/2)},{0.52*sin(\tone/2)}) {$\theta_1$};
  \node[anchor=west,inner sep=0.5pt,font=\scriptsize] at ({0.85*cos((\tone+\thP)/2)},{0.85*sin((\tone+\thP)/2)}) {$\theta$};
\end{tikzpicture}%
}
\caption{(a)}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·17（b）圆心在极点 O 的圆，半径 r；虚线 OP，圆上一点 P
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\rr{1.55}
  \def\th{48}
  \coordinate (O) at (0,0);
  \coordinate (P) at ({\th}:{\rr});
  \draw[line width=0.65pt,-latex] (-{\rr*0.35},0) -- ({\rr+0.85},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.85pt] (O) circle ({\rr});
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.55pt] (O) -- (P);
  \fill (O) circle (1.1pt);
  \fill (P) circle (1.15pt);
  \node[anchor=north east,inner sep=1.5pt,font=\small] at (O) {$O$};
  \node[anchor=south west,inner sep=1.5pt,font=\small] at (P) {$P$};
  \node[anchor=south east,inner sep=1pt,font=\small] at ({0.52*\rr*cos(\th)},{0.52*\rr*sin(\th)}) {$r$};
\end{tikzpicture}%
}
\caption{(b)}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·17（c）过极点且圆心 C 在极轴上的圆；直径沿极轴，圆上点 P，连 OP 与直径远端
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\rr{1.25}
  \def\th{42}
  \coordinate (O) at (0,0);
  \coordinate (C) at ({\rr},0);
  \coordinate (A) at ({2*\rr},0);
  \coordinate (P) at ({2*\rr*cos(\th)},{2*\rr*sin(\th)});
  \draw[line width=0.65pt,-latex] (-0.35,0) -- ({2*\rr+0.95},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.85pt] (C) circle ({\rr});
  \draw[line width=0.75pt] (O) -- (P);
  \draw[line width=0.7pt] (P) -- (A);
  \fill (O) circle (1.1pt);
  ...
  \fill (P) circle (1.15pt);
  \node[anchor=north east,inner sep=1.5pt,font=\small] at (O) {$O$};
  \node[anchor=north,inner sep=1.6pt,font=\small] at (C) {$C$};
  \node[anchor=south,inner sep=1.5pt,font=\small] at (P) {$P$};
\end{tikzpicture}%
}
\caption{(c)}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·17（d）过极点且圆心在极垂线上的圆；竖直直径标 a，圆上点 P，虚线 OP 与至直径顶端
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{2.2}
  \def\th{128}
  \coordinate (O) at (0,0);
  \coordinate (Top) at (0,{\aa});
  \coordinate (C) at (0,{\aa/2});
  \coordinate (P) at ({\th}:{\aa*sin(\th)});
  \draw[line width=0.65pt,-latex] (-0.15,0) -- ({\aa*0.85},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.85pt] (C) circle ({\aa/2});
  \draw[line width=0.75pt] (O) -- (Top);
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.55pt] (O) -- (P);
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.55pt] (P) -- (Top);
  \fill (O) circle (1.1pt);
  \fill (P) circle (1.15pt);
  \node[anchor=north,inner sep=1.8pt,font=\small] at (O) {$O$};
  \node[anchor=east,inner sep=1.5pt,font=\small] at (P) {$P$};
  \node[anchor=west,inner sep=1.5pt,font=\small] at (0,{\aa*0.5}) {$a$};
\end{tikzpicture}%
}
\caption{(d)}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·18 圆锥曲线极坐标：焦点 O(F)，准线 l，OH=p；点 P，OP=ρ，PD⊥l，PQ⊥Ox，∠xOP=θ
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\pp{1.55}
  \def\rhop{2.85}
  \def\th{48}
  \coordinate (O) at (0,0);
  \coordinate (H) at ({-\pp},0);
  \coordinate (P) at ({\th}:{\rhop});
  \coordinate (Q) at ({\rhop*cos(\th)},0);
  \coordinate (D) at ({-\pp},{\rhop*sin(\th)});
  \draw[line width=0.65pt,-latex] ({-\pp-0.55},0) -- ({\rhop*cos(\th)+1.35},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.85pt] ({-\pp},{-0.55}) -- ({-\pp},{\rhop*sin(\th)+0.85}) node[anchor=south,inner sep=1.5pt] {$l$};
  \draw[line width=0.75pt] (O) -- (P);
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.5pt] (P) -- (Q);
  \draw[dash pattern=on 1.8pt off 1.2pt,line width=0.5pt] (P) -- (D);
  \draw[line width=0.4pt] (0.55,0) arc (0:\th:0.55);
  \fill (O) circle (1.15pt);
  ...
  \fill (Q) circle (1.05pt);
  \fill (D) circle (1.05pt);
  \node[anchor=north west,inner sep=1.2pt,font=\small] at (0.02,-0.02) {$O(F)$};
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (H) {$H$};
  \node[anchor=south west,inner sep=1.5pt,font=\small] at (P) {$P$};
  \node[anchor=north,inner sep=1.5pt,font=\small] at (Q) {$Q$};
  \node[anchor=east,inner sep=1.5pt,font=\small] at (D) {$D$};
  \node[anchor=north,inner sep=3pt,font=\small] at ({-\pp*0.5},0) {$p$};
  \node[anchor=south east,inner sep=1pt,font=\small] at ({0.55*\rhop*cos(\th)},{0.55*\rhop*sin(\th)}) {$\rho$};
  \node[anchor=west,inner sep=0.8pt,font=\scriptsize] at ({0.62*cos(\th/2)},{0.62*sin(\th/2)}) {$\theta$};
\end{tikzpicture}%
}
\caption{图7·18}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·19 抛物线 ρ=p/(1-cosθ)，焦点为极点 O；焦弦 PQ，P(ρ,θ)、Q(ρ₁,θ+π)，夹角 θ
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\pp{1.35}
  \def\th{42}
  \pgfmathsetmacro{\rhoP}{\pp/(1-cos(\th))}
  \pgfmathsetmacro{\rhoQ}{\pp/(1-cos(\th+180))}
  \coordinate (O) at (0,0);
  \coordinate (P) at ({\th}:{\rhoP});
  \coordinate (Q) at ({\th+180}:{\rhoQ});
  \draw[line width=0.65pt,-latex] ({-\pp*0.85},0) -- ({\rhoP*cos(\th)+0.9},0) node[anchor=west,inner sep=1.6pt] {$x$};
  \draw[line width=0.9pt,domain=-2.35:2.35,samples=80,smooth]
    plot ({(\x*\x)/(2*\pp) - \pp/2}, {\x});
  \draw[line width=0.75pt] (Q) -- (P);
  \draw[line width=0.4pt] (0.48,0) arc (0:\th:0.48);
  \fill (O) circle (1.15pt);
  \fill (P) circle (1.15pt);
  \fill (Q) circle (1.15pt);
  \node[anchor=south east,inner sep=1.4pt,font=\small] at (O) {$O$};
  \node[anchor=west,inner sep=1.5pt,font=\small] at (P) {$P(\rho,\theta)$};
  \node[anchor=west,inner sep=1.5pt,font=\small] at (Q) {$Q(\rho_1,\theta+\pi)$};
  \node[anchor=west,inner sep=0.6pt,font=\scriptsize] at ({0.55*cos(\th/2)},{0.55*sin(\th/2)}) {$\theta$};
\end{tikzpicture}%
}
\caption{图7·19}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·20 凸轮与从动杆：凸轮顺时针转动，推动竖直从动杆在导槽中作往复直线运动
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  % pear cam: wide bottom, pointed top
  \draw[line width=0.85pt,fill=white]
    plot[domain=0:360,samples=160,smooth cycle]
    ({0.78*(1.20-0.38*cos(\x))*sin(\x)},
     {0.92*(1.15-0.28*cos(\x))*cos(\x) - 0.12});
  % shaft + keyway at bottom
  \fill[white] (0,-0.42) circle (0.24);
  \draw[line width=0.65pt] (0,-0.42) circle (0.24);
  \fill[white] (-0.055,-0.42-0.24) rectangle (0.055,-0.42-0.38);
  \draw[line width=0.55pt] (-0.055,-0.42-0.16) -- (-0.055,-0.42-0.38) -- (0.055,-0.42-0.38) -- (0.055,-0.42-0.16);
  % clockwise arrow
  \draw[line width=0.55pt,-latex] (1.15,-0.05) arc (0:-80:1.22);
  % guide bar
  \fill[pattern=north east lines,pattern color=black!55] (-1.0,1.42) rectangle (1.0,1.82);
  \draw[line width=0.7pt] (-1.0,1.42) rectangle (1.0,1.82);
  \fill[white] (-0.11,1.39) rectangle (0.11,1.85);
  \draw[line width=0.55pt] (-0.11,1.42) -- (-0.11,1.82);
  \draw[line width=0.55pt] (0.11,1.42) -- (0.11,1.82);
  % follower rod
  \draw[line width=0.85pt] (0,1.05) -- (0,2.42);
  \fill[white] (-0.09,1.05) -- (0,0.90) -- (0.09,1.05) -- cycle;
  \draw[line width=0.7pt] (-0.09,1.05) -- (0,0.90) -- (0.09,1.05) -- cycle;
  \node[anchor=south,inner sep=2pt,font=\small] at (0,2.42) {从动杆};
  \node[anchor=center,font=\small] at (0,-0.95) {凸轮};
\end{tikzpicture}%
}
\caption{图7·20}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·21 等速螺线 ρ=ρ₀+aθ（a＞0）：极点 O，极轴 Ox，起点 P₀(ρ₀,0)，射线 OM 与螺线交于 P(ρ,θ)，标 θ
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\rzero{1.15}
  \def\aa{0.28}
  \def\tmax{300}
  \def\th{38}
  \pgfmathsetmacro{\rp}{\rzero+\aa*(\th*pi/180)}
  \pgfmathsetmacro{\rmax}{\rzero+\aa*(\tmax*pi/180)}
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.55pt] ({-0.85},0) -- (0,0);
  \draw[line width=0.7pt,-latex] (0,0) -- ({\rmax+0.95},0) node[anchor=west,inner sep=1.5pt] {$x$};
  \draw[line width=0.85pt,domain=0:\tmax,samples=120,smooth]
    plot ({(\rzero+\aa*(\x*pi/180))*cos(\x)},{(\rzero+\aa*(\x*pi/180))*sin(\x)});
  \draw[line width=0.7pt,-latex] (0,0) -- ({\th}:{\rp+0.85}) node[anchor=south west,inner sep=1.2pt] {$M$};
  \draw[line width=0.4pt] (0.42,0) arc (0:\th:0.42);
  \fill (0,0) circle (1.15pt);
  \fill ({\rzero},0) circle (1.1pt);
  \fill ({\th}:{\rp}) circle (1.15pt);
  \node[anchor=north east,inner sep=1.4pt,font=\small] at (0,0) {$O$};
  \node[anchor=north,inner sep=1.6pt,font=\small] at ({\rzero},0) {$P_0(\rho_0,0)$};
  \node[anchor=south east,inner sep=1.2pt,font=\small] at ({\th}:{\rp}) {$P(\rho,\theta)$};
  \node[anchor=west,inner sep=0.6pt,font=\scriptsize] at ({0.48*cos(\th/2)},{0.48*sin(\th/2)}) {$\theta$};
\end{tikzpicture}%
}
\caption{图7·21}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·22 等速螺线 ρ=ρ₀+aθ（a＜0）：自 P₀ 起极径随极角增大而均匀减小，射线 OM 交螺线于 P，标 θ
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\rzero{2.70}
  \def\aa{0.28}
  \def\tmax{480}
  \def\th{42}
  \pgfmathsetmacro{\rp}{\rzero-\aa*(\th*pi/180)}
  \pgfmathsetmacro{\rend}{\rzero-\aa*(\tmax*pi/180)}
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.55pt] (0,0) -- ({\rzero+0.35},0);
  \draw[line width=0.7pt,-latex] ({\rzero+0.35},0) -- ({\rzero+0.95},0) node[anchor=west,inner sep=1.5pt] {$x$};
  \draw[line width=0.85pt,domain=0:\tmax,samples=180,smooth]
    plot ({(\rzero-\aa*(\x*pi/180))*cos(\x)},{(\rzero-\aa*(\x*pi/180))*sin(\x)});
  \draw[line width=0.7pt,-latex] (0,0) -- ({\th}:{\rp+0.45}) node[anchor=south west,inner sep=1.2pt] {$M$};
  \draw[line width=0.4pt] (0.40,0) arc (0:\th:0.40);
  \fill (0,0) circle (1.15pt);
  \fill ({\rzero},0) circle (1.1pt);
  \fill ({\th}:{\rp}) circle (1.15pt);
  \node[anchor=north east,inner sep=1.4pt,font=\small] at (0,0) {$O$};
  \node[anchor=north,inner sep=1.5pt,font=\small] at ({\rzero},0) {$P_0$};
  \node[anchor=south,inner sep=1.4pt,font=\small] at ({\th}:{\rp}) {$P$};
  \node[anchor=west,inner sep=0.5pt,font=\scriptsize] at ({0.46*cos(\th/2)},{0.46*sin(\th/2)}) {$\theta$};
\end{tikzpicture}%
}
\caption{图7·22}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·23 ρ=3θ：极坐标网格，实线为 ρ、θ 均正的螺线，虚线为均负且关于极垂线对称的部分
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\sc{0.105}
  \def\Rmax{19}
  \pgfmathsetmacro{\Rg}{\sc*\Rmax}
  \foreach \rr in {1,2,...,19} {
    \draw[line width=0.22pt] (0,0) circle ({\sc*\rr});
  }
  \foreach \ang in {30,60,90,120,150,180,210,240,270,300,330} {
    \draw[line width=0.22pt] (0,0) -- ({\ang}:{\Rg});
  }
  \draw[line width=0.55pt] ({-\Rg},0) -- (0,0);
  \draw[line width=0.7pt,-latex] (0,0) -- ({\Rg+0.42},0) node[anchor=west,inner sep=1.3pt] {$x$};
  \fill (0,0) circle (1.05pt);
  \node[anchor=north east,inner sep=1.1pt,font=\small] at (-0.02,-0.02) {$O$};
  \foreach \ang/\lab in {
    30/\frac{\pi}{6},60/\frac{\pi}{3},90/\frac{\pi}{2},120/\frac{2\pi}{3},150/\frac{5\pi}{6},
    180/\pi,210/\frac{7\pi}{6},240/\frac{4\pi}{3},270/\frac{3\pi}{2},300/\frac{5\pi}{3},330/\frac{11\pi}{6}} {
    \node[font=\scriptsize,inner sep=0.4pt] at ({\ang}:{\Rg+0.28}) {$\lab$};
  }
  \draw[line width=0.95pt,domain=0:360,samples=180,smooth]
    plot ({(\sc*3*(\x*pi/180))*cos(\x)},{(\sc*3*(\x*pi/180))*sin(\x)});
  \draw[line width=0.95pt,domain=360:400,samples=30,smooth]
    plot ({(\sc*3*(\x*pi/180))*cos(\x)},{(\sc*3*(\x*pi/180))*sin(\x)});
  \draw[dash pattern=on 1.7pt off 1.15pt,line width=0.85pt,domain=0:360,samples=180,smooth]
    plot ({-(\sc*3*(\x*pi/180))*cos(\x)},{(\sc*3*(\x*pi/180))*sin(\x)});
  \draw[dash pattern=on 1.7pt off 1.15pt,line width=0.85pt,domain=360:400,samples=30,smooth]
    plot ({-(\sc*3*(\x*pi/180))*cos(\x)},{(\sc*3*(\x*pi/180))*sin(\x)});
\end{tikzpicture}%
}
\caption{图7·23}
...
\centering
\fitfig{%
% fig-alt: 图7·24 三圈等速螺线：起点 A(20,0)、终点 B(35,6π) 均在极轴上，极点 O，极轴 Ox
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\sc{0.058}
  \def\rA{20}
  \def\rB{35}
  \pgfmathsetmacro{\aa}{5/(2*pi)}
  \draw[line width=0.65pt,-latex] ({-\sc*3},0) -- ({\sc*\rB+0.90},0) node[anchor=west,inner sep=1.5pt] {$x$};
  \draw[line width=0.9pt,domain=0:1080,samples=280,smooth]
    plot ({(\sc*(\rA+\aa*(\x*pi/180)))*cos(\x)},{(\sc*(\rA+\aa*(\x*pi/180)))*sin(\x)});
  \fill (0,0) circle (1.15pt);
  \fill ({\sc*\rA},0) circle (1.1pt);
  \fill ({\sc*\rB},0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.3pt,font=\small] at (0,0) {$O$};
  \node[anchor=north,inner sep=1.5pt,font=\small] at ({\sc*\rA},0) {$A$};
  \node[anchor=south,inner sep=1.5pt,font=\small] at ({\sc*\rB},0) {$B$};
\end{tikzpicture}%
}
\caption{图7·24}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 图7·25 凸轮轮廓：AB 等速螺线 A(25,0)→B(35,π/2)，BC 圆弧 ρ=35，CA 等速螺线 C(35,4π/3)→A(25,2π)；虚线 OB、OC，标 π/2、5π/6
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\sc{0.050}
  \pgfmathsetmacro{\rA}{25*\sc}
  \pgfmathsetmacro{\rB}{35*\sc}
  \draw[line width=0.65pt,-latex] ({-0.30},0) -- ({\rB+1.00},0) node[anchor=west,inner sep=1.4pt] {$x$};
  % AB
  \draw[line width=0.9pt,domain=0:90,samples=70,smooth]
    plot ({(\sc*(25+(20/pi)*(\x*pi/180)))*cos(\x)},{(\sc*(25+(20/pi)*(\x*pi/180)))*sin(\x)});
  % BC
  \draw[line width=0.9pt] (90:\rB) arc (90:240:\rB);
  % CA
  \draw[line width=0.9pt,domain=240:360,samples=80,smooth]
    plot ({(\sc*(55-(15/pi)*(\x*pi/180)))*cos(\x)},{(\sc*(55-(15/pi)*(\x*pi/180)))*sin(\x)});
  \draw[dash pattern=on 1.7pt off 1.15pt,line width=0.55pt] (0,0) -- (90:{\rB+0.12});
  \draw[dash pattern=on 1.7pt off 1.15pt,line width=0.55pt] (0,0) -- (240:{\rB+0.12});
  \draw[line width=0.4pt] (0.52,0) arc (0:90:0.52);
  \draw[line width=0.4pt] (90:0.70) arc (90:240:0.70);
  \fill (0,0) circle (1.15pt);
  \fill ({\rA},0) circle (1.1pt);
  \fill (90:\rB) circle (1.1pt);
  \fill (240:\rB) circle (1.1pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (0,0) {$O$};
  \node[anchor=south,inner sep=1.1pt,font=\small] at ({\rA*0.48},0.06) {$(25,0)$};
  \node[anchor=north,inner sep=1.4pt,font=\small] at ({\rA},-0.02) {$A(25,2\pi)$};
  \node[anchor=south west,inner sep=1.3pt,font=\small] at (90:\rB) {$B\!\left(35,\frac{\pi}{2}\right)$};
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (240:\rB) {$C\!\left(35,\frac{4\pi}{3}\right)$};
  \node[anchor=south west,inner sep=0.5pt,font=\scriptsize] at ({0.60*cos(45)},{0.60*sin(45)}) {$\frac{\pi}{2}$};
  \node[anchor=east,inner sep=0.5pt,font=\scriptsize] at ({0.80*cos(165)},{0.80*sin(165)}) {$\frac{5\pi}{6}$};
\end{tikzpicture}%
}
\caption{图7·25}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: （第1题）内外圆 φ43、φ102，等速螺线自 A（内圆与极轴交点）转 5π/8 至外圆上 B；虚线 OB，标 5π/8
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\sc{0.028}
  \pgfmathsetmacro{\ri}{21.5*\sc}
  \pgfmathsetmacro{\ro}{51*\sc}
  \pgfmathsetmacro{\thB}{112.5}
  \pgfmathsetmacro{\aa}{(51-21.5)/(5*pi/8)}
  \draw[line width=0.65pt,-latex] ({-0.25},0) -- ({\ro+0.75},0) node[anchor=west,inner sep=1.4pt] {$x$};
  \draw[line width=0.7pt] (0,0) circle ({\ri});
  \draw[line width=0.7pt] (0,0) circle ({\ro});
  \draw[line width=0.9pt,domain=0:\thB,samples=80,smooth]
    plot ({(\sc*(21.5+\aa*(\x*pi/180)))*cos(\x)},{(\sc*(21.5+\aa*(\x*pi/180)))*sin(\x)});
  \draw[dash pattern=on 1.7pt off 1.15pt,line width=0.55pt] (0,0) -- ({\thB}:{\ro+0.12});
  \draw[line width=0.4pt,<->] (0.55,0) arc (0:\thB:0.55);
  \fill (0,0) circle (1.1pt);
  \fill ({\ri},0) circle (1.05pt);
  \fill ({\thB}:{\ro}) circle (1.05pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (0,0) {$O$};
  \node[anchor=north,inner sep=1.4pt,font=\small] at ({\ri},0) {$A$};
  \node[anchor=south west,inner sep=1.2pt,font=\small] at ({\thB}:{\ro}) {$B$};
  \node[anchor=south,inner sep=0.8pt,font=\small] at ({0}:{\ri*0.55}) {$\phi 43$};
  \node[anchor=west,inner sep=1pt,font=\small] at ({-10}:{\ro*0.72}) {$\phi 102$};
  \node[anchor=west,inner sep=0.5pt,font=\scriptsize] at ({0.62*cos(\thB/2)},{0.62*sin(\thB/2)}) {$\dfrac{5\pi}{8}$};
\end{tikzpicture}%
}
\caption{（第1题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: （第2题）等速螺线弧 ACB：OA=60，OB=65，∠AOB=30°；虚线 OB，曲线上标 ACB
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\sc{0.028}
  \pgfmathsetmacro{\rA}{60*\sc}
  \pgfmathsetmacro{\rB}{65*\sc}
  \def\thB{30}
  \pgfmathsetmacro{\aa}{5/(\thB*pi/180)}
  \draw[line width=0.65pt] ({-0.2},0) -- ({\rB+0.35},0);
  \draw[line width=0.55pt] (0,0) -- ({\rA},0);
  \draw[dash pattern=on 1.7pt off 1.15pt,line width=0.55pt] (0,0) -- ({\thB}:{\rB+0.12});
  \draw[line width=0.9pt,domain=0:\thB,samples=40,smooth]
    plot ({(\sc*(60+\aa*(\x*pi/180)))*cos(\x)},{(\sc*(60+\aa*(\x*pi/180)))*sin(\x)});
  \draw[line width=0.4pt] (0.48,0) arc (0:\thB:0.48);
  \fill (0,0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (0,0) {$O$};
  \fill ({\rA},0) circle (1.05pt);
  \fill ({\thB}:{\rB}) circle (1.05pt);
  \node[anchor=north,inner sep=1.5pt,font=\small] at ({\rA},0) {$A$};
  \node[anchor=south west,inner sep=1.2pt,font=\small] at ({\thB}:{\rB}) {$B$};
  \node[anchor=south,inner sep=1.2pt,font=\small] at ({15}:{(\sc*(60+\aa*(15*pi/180)))+0.12}) {$ACB$};
  \node[anchor=north,inner sep=1.5pt,font=\small] at ({\rA*0.5},0) {$60$};
  \node[anchor=east,inner sep=1pt,font=\small] at ({(\thB+8)}:{\rB*0.55}) {$65$};
  \node[anchor=west,inner sep=0.5pt,font=\scriptsize] at ({0.55*cos(\thB/2)},{0.55*sin(\thB/2)}) {$30^\circ$};
\end{tikzpicture}%
}
\caption{（第2题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: （第3题）凸轮：圆弧 ABC 半径 100，螺线 CDE 至 E 多出 10；顺时针转；右侧从动杆穿剖面导槽；尺寸 100、10
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\R{1.55}
  \def\dE{0.22}
  \pgfmathsetmacro{\RE}{\R+\dE}
  \draw[dash pattern=on 1.5pt off 1.0pt,line width=0.45pt] ({-\RE-0.35},0) -- ({\R+2.65},0);
  \draw[dash pattern=on 1.5pt off 1.0pt,line width=0.45pt] (0,{-\R-0.2}) -- (0,{\R+0.5});
  \draw[line width=0.9pt] (180:\R) arc (180:360:\R);
  \draw[line width=0.9pt,domain=0:180,samples=90,smooth]
    plot ({(\R+\dE*\x/180)*cos(\x)},{(\R+\dE*\x/180)*sin(\x)});
  \draw[line width=0.55pt] (180:\R) -- (180:\RE);
  \draw[line width=0.5pt,-latex] (70:{\R+0.28}) arc (70:20:{\R+0.28});
  \draw[line width=0.7pt] ({\R},-0.08) -- ({\R+0.35},-0.08) -- ({\R+0.35},0.08) -- ({\R},0.08);
  \fill[pattern=north east lines,pattern color=black!55] ({\R+1.15},0.22) rectangle ({\R+1.85},0.95);
  \draw[line width=0.65pt] ({\R+1.15},0.22) rectangle ({\R+1.85},0.95);
  \fill[pattern=north east lines,pattern color=black!55] ({\R+1.15},-0.95) rectangle ({\R+1.85},-0.22);
  \draw[line width=0.65pt] ({\R+1.15},-0.95) rectangle ({\R+1.85},-0.22);
  \fill[white] ({\R+1.12},-0.09) rectangle ({\R+1.88},0.09);
  \draw[line width=0.55pt] ({\R+1.15},0.09) -- ({\R+1.85},0.09);
  \draw[line width=0.55pt] ({\R+1.15},-0.09) -- ({\R+1.85},-0.09);
  \draw[line width=0.85pt] ({\R},0) -- ({\R+2.35},0);
  \draw[line width=0.45pt,latex-latex] (0,0.35) -- ({\R},0.35);
  \node[anchor=south,inner sep=1pt,font=\small] at ({\R/2},0.35) {$100$};
  \draw[line width=0.45pt,latex-latex] (180:\R) -- (180:\RE);
  \node[anchor=east,inner sep=1.2pt,font=\small] at (180:{(\R+\RE)/2}) {$10$};
  \fill (0,0) circle (1.15pt);
  \fill (180:\R) circle (1.05pt);
  \fill (270:\R) circle (1.05pt);
  \fill (0:\R) circle (1.05pt);
  \fill (90:\R) circle (1.05pt);
  \fill (180:\RE) circle (1.05pt);
  \node[anchor=north west,inner sep=1.2pt,font=\small] at (0,0) {$O$};
  \node[anchor=north,inner sep=1.5pt,font=\small] at (180:\R) {$A$};
  \node[anchor=north,inner sep=1.5pt,font=\small] at (270:\R) {$B$};
  \node[anchor=south west,inner sep=1.3pt,font=\small] at (0:\R) {$C$};
  \node[anchor=south,inner sep=1.4pt,font=\small] at (90:\R) {$D$};
  \node[anchor=south east,inner sep=1.2pt,font=\small] at (180:\RE) {$E$};
\end{tikzpicture}%
}
\caption{（第3题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: （第4题）凸轮轮廓：OA=4，OC=18，∠AOC=11π/8；ABC 等速推出螺线，CA 等速退回；顺时针转箭头
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\sc{0.095}
  \pgfmathsetmacro{\rA}{4*\sc}
  \pgfmathsetmacro{\rC}{18*\sc}
  \def\thC{247.5}
  \draw[dash pattern=on 1.6pt off 1.1pt,line width=0.5pt] (0,0) -- ({\rA+0.15},0);
  \draw[line width=0.65pt,-latex] ({\rA+0.15},0) -- ({\rA+0.85},0);
  \draw[line width=0.9pt,domain=0:\thC,samples=160,smooth]
    plot ({(\sc*(4+(112/(11*pi))*(\x*pi/180)))*cos(\x)},{(\sc*(4+(112/(11*pi))*(\x*pi/180)))*sin(\x)});
  \draw[line width=0.9pt,domain=\thC:360,samples=70,smooth]
    plot ({(\sc*(18-(112/(5*pi))*((\x-\thC)*pi/180)))*cos(\x)},{(\sc*(18-(112/(5*pi))*((\x-\thC)*pi/180)))*sin(\x)});
  \draw[line width=0.7pt] (0,0) -- ({\thC}:{\rC});
  \draw[line width=0.4pt] (0.38,0) arc (0:\thC:0.38);
  \draw[line width=0.5pt,-latex] (105:{\rC*0.72}) arc (105:55:{\rC*0.72});
  \fill (0,0) circle (1.15pt);
  \fill ({\rA},0) circle (1.05pt);
  \fill ({\thC}:{\rC}) circle (1.05pt);
  \pgfmathsetmacro{\rB}{\sc*(4+(112/(11*pi))*pi)}
  \fill (180:{\rB}) circle (1.0pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (0,0) {$O$};
  \node[anchor=south,inner sep=1.3pt,font=\small] at ({\rA},0) {$A$};
  \node[anchor=east,inner sep=1.4pt,font=\small] at (180:{\rB}) {$B$};
  \node[anchor=north,inner sep=1.3pt,font=\small] at ({\thC}:{\rC}) {$C$};
  \node[anchor=north,inner sep=1.2pt,font=\small] at ({\rA*0.5},0) {$4$};
  \node[anchor=west,inner sep=1.2pt,font=\small] at ({\thC}:{\rC*0.52}) {$18$};
  \node[anchor=south west,inner sep=0.4pt,font=\scriptsize] at ({0.48*cos(100)},{0.48*sin(100)}) {$\dfrac{11}{8}\pi$};
\end{tikzpicture}%
}
\caption{（第4题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: （第9（2）题）ρ=a(cscθ+1)：上支峰 2a，下支尖点于 O；虚线极垂线标 90°、a、2a，极轴 Ox
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{0.95}
  \begin{scope}
    \clip (-3.2,-0.35) rectangle (3.2,2.55);
    \draw[line width=0.9pt,domain=18:162,samples=120,smooth]
      plot ({(\aa*(1+1/sin(\x)))*cos(\x)},{(\aa*(1+1/sin(\x)))*sin(\x)});
    \draw[line width=0.9pt,domain=198:342,samples=120,smooth]
      plot ({(\aa*(1+1/sin(\x)))*cos(\x)},{(\aa*(1+1/sin(\x)))*sin(\x)});
  \end{scope}
  \draw[dash pattern=on 1.5pt off 1.05pt,line width=0.5pt] (0,0) -- (0,2.45);
  \draw[line width=0.55pt] (-2.85,0) -- (0,0);
  \draw[line width=0.7pt,-latex] (0,0) -- (3.05,0) node[anchor=west,inner sep=1.4pt] {$x$};
  \fill (0,0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.3pt,font=\small] at (0,0) {$O$};
  \node[anchor=south,inner sep=1.2pt,font=\small] at (0,2.45) {$90^\circ$};
  \node[anchor=west,inner sep=1.5pt,font=\small] at (0,{\aa}) {$a$};
  \node[anchor=west,inner sep=1.5pt,font=\small] at (0,{2*\aa}) {$2a$};
  \draw[line width=0.35pt] (-0.08,{\aa}) -- (0.08,{\aa});
  \draw[line width=0.35pt] (-0.08,{2*\aa}) -- (0.08,{2*\aa});
\end{tikzpicture}%
}
\caption{（第9（2）题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: （第9（3）题）ρ=a cos(2θ/3) 示意：过极点内外四对圆，尖端 A、C、B、D、E、D′；虚线轴与 45°、135° 射线
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\R{0.92}
  \def\rr{0.55}
  \pgfmathsetmacro{\Rout}{2*\R}
  \pgfmathsetmacro{\rl}{\Rout+0.35}
  \draw[dash pattern=on 1.4pt off 1.0pt,line width=0.45pt] ({-\rl},0) -- ({\rl},0);
  \draw[dash pattern=on 1.4pt off 1.0pt,line width=0.45pt] (0,{-\rl}) -- (0,{\rl});
  \draw[dash pattern=on 1.4pt off 1.0pt,line width=0.4pt] (0,0) -- (45:{\rl});
  \draw[dash pattern=on 1.4pt off 1.0pt,line width=0.4pt] (0,0) -- (135:{\rl});
  \draw[line width=0.65pt,-latex] ({\rl-0.05},0) -- ({\rl+0.35},0) node[anchor=west,inner sep=1.2pt] {$x$};
  \draw[line width=0.82pt] ({\R},0) circle ({\R});
  \draw[line width=0.82pt] ({-\R},0) circle ({\R});
  \draw[line width=0.82pt] (0,{\R}) circle ({\R});
  \draw[line width=0.82pt] (0,{-\R}) circle ({\R});
  \draw[line width=0.75pt] ({\rr},0) circle ({\rr});
  \draw[line width=0.75pt] ({-\rr},0) circle ({\rr});
  \draw[line width=0.75pt] (0,{\rr}) circle ({\rr});
  \draw[line width=0.75pt] (0,{-\rr}) circle ({\rr});
  \fill (0,0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.1pt,font=\small] at (0,0) {$O$};
  \node[anchor=south,inner sep=1.2pt,font=\small] at ({\Rout},0) {$A$};
  \node[anchor=south,inner sep=1.0pt,font=\small] at ({2*\rr},0) {$C$};
  \node[anchor=south,inner sep=1.0pt,font=\small] at ({-2*\rr},0) {$E$};
  \node[anchor=west,inner sep=1.2pt,font=\small] at (0,{2*\rr}) {$B$};
  \node[anchor=west,inner sep=1.2pt,font=\small] at (0,{\Rout}) {$D$};
  \node[anchor=west,inner sep=1.2pt,font=\small] at (0,{-\Rout}) {$D'$};
  \node[anchor=east,inner sep=0.8pt,font=\scriptsize] at (45:{\rl}) {$45^\circ$};
  \node[anchor=south,inner sep=1.0pt,font=\scriptsize] at (90:{\rl}) {$90^\circ$};
  \node[anchor=west,inner sep=0.8pt,font=\scriptsize] at (135:{\rl}) {$135^\circ$};
\end{tikzpicture}%
}
\caption{（第9（3）题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: （第9（4）题）双曲螺线 ρ=π/θ：正负极角两支，渐近水平，向极点 O 旋进；极轴 Ox
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\sc{0.42}
  \begin{scope}
    \clip (-3.4,-2.6) rectangle (3.6,2.8);
    \draw[line width=0.88pt,domain=28:1080,samples=280,smooth]
      plot ({(\sc*pi/(\x*pi/180))*cos(\x)},{(\sc*pi/(\x*pi/180))*sin(\x)});
    \draw[line width=0.88pt,domain=-1080:-28,samples=280,smooth]
      plot ({(\sc*pi/(\x*pi/180))*cos(\x)},{(\sc*pi/(\x*pi/180))*sin(\x)});
  \end{scope}
  \draw[line width=0.65pt,-latex] (-0.15,0) -- (3.35,0) node[anchor=west,inner sep=1.4pt] {$x$};
  \fill (0,0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.3pt,font=\small] at (0,0) {$O$};
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: （第9（5）题）对数螺线 ρ=e^{2θ}：自极点外旋；近极虚线，外侧实线；极轴 Ox
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\sc{0.095}
  % many turns: θ∈[-400°,95°]; dashed near pole, solid outer
  \draw[dash pattern=on 1.4pt off 1.0pt,line width=0.7pt,domain=-400:-60,samples=220,smooth]
    plot ({(\sc*exp(2*(\x*pi/180)))*cos(\x)},{(\sc*exp(2*(\x*pi/180)))*sin(\x)});
  \draw[line width=0.88pt,domain=-60:95,samples=140,smooth]
    plot ({(\sc*exp(2*(\x*pi/180)))*cos(\x)},{(\sc*exp(2*(\x*pi/180)))*sin(\x)});
  \draw[line width=0.65pt,-latex] (-0.25,0) -- (2.85,0) node[anchor=west,inner sep=1.4pt] {$x$};
  \fill (0,0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.3pt,font=\small] at (0,0) {$O$};
\end{tikzpicture}%
}
...
\centering
\fitfig{%
% fig-alt: （第14题）凸轮：ABE/ADF 圆弧 R=20，EC/FC 等速螺线至 C，∠EOC=65°，OC=33；右侧从动杆与剖面导槽
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\sc{0.048}
  \pgfmathsetmacro{\rR}{20*\sc}
  \pgfmathsetmacro{\rC}{33*\sc}
  \def\thE{115}
  \def\thF{245}
  \draw[dash pattern=on 1.5pt off 1.05pt,line width=0.45pt] ({-\rC-0.25},0) -- ({\rR+2.55},0);
  \draw[dash pattern=on 1.5pt off 1.05pt,line width=0.45pt] (0,{-\rR-0.25}) -- (0,{\rR+0.35});
  \draw[line width=0.9pt] (0:\rR) arc (0:\thE:\rR);
  \draw[line width=0.9pt] (0:\rR) arc (0:-115:\rR);
  \draw[line width=0.9pt,domain=\thE:180,samples=50,smooth]
    plot ({(\sc*(20+(13/65)*(\x-\thE)))*cos(\x)},{(\sc*(20+(13/65)*(\x-\thE)))*sin(\x)});
  \draw[line width=0.9pt,domain=180:\thF,samples=50,smooth]
    plot ({(\sc*(20+(13/65)*(\thF-\x)))*cos(\x)},{(\sc*(20+(13/65)*(\thF-\x)))*sin(\x)});
  \draw[dash pattern=on 1.5pt off 1.05pt,line width=0.5pt] (0,0) -- ({\thE}:{\rR+0.08});
  \draw[line width=0.4pt,latex-latex] (180:0.42) arc (180:\thE:0.42);
  \node[anchor=east,inner sep=0.6pt,font=\scriptsize] at (148:0.58) {$65^\circ$};
  \draw[line width=0.4pt,-latex] (0.15,0.12) -- (18:\rR);
  \node[anchor=south west,inner sep=0.8pt,font=\small] at (0.2,0.15) {$R\,20$};
  \draw[line width=0.7pt] ({\rR},-0.07) -- ({\rR+0.32},-0.07) -- ({\rR+0.32},0.07) -- ({\rR},0.07);
  \fill[pattern=north east lines,pattern color=black!55] ({\rR+1.05},0.18) rectangle ({\rR+1.75},0.78);
  \draw[line width=0.6pt] ({\rR+1.05},0.18) rectangle ({\rR+1.75},0.78);
  \fill[pattern=north east lines,pattern color=black!55] ({\rR+1.05},-0.78) rectangle ({\rR+1.75},-0.18);
  \draw[line width=0.6pt] ({\rR+1.05},-0.78) rectangle ({\rR+1.75},-0.18);
  \draw[line width=0.75pt] ({\rR},0) -- ({\rR+2.15},0);
  \node[anchor=south,inner sep=1.5pt,font=\small] at ({\rR+1.4},0.78) {从动杆};
  \fill (0,0) circle (1.1pt);
  \fill ({\rR},0) circle (1.0pt);
  \fill (90:\rR) circle (1.0pt);
  \fill (270:\rR) circle (1.0pt);
  \fill ({\thE}:\rR) circle (1.0pt);
  \fill ({\thF}:\rR) circle (1.0pt);
  \fill (180:\rC) circle (1.0pt);
  \node[anchor=north west,inner sep=1.1pt,font=\small] at (0,0) {$O$};
  \node[anchor=north,inner sep=1.3pt,font=\small] at ({\rR},0) {$A$};
  \node[anchor=south,inner sep=1.3pt,font=\small] at (90:\rR) {$B$};
  \node[anchor=east,inner sep=1.3pt,font=\small] at (180:\rC) {$C$};
  \node[anchor=north,inner sep=1.2pt,font=\small] at (270:\rR) {$D$};
  \node[anchor=south east,inner sep=1.1pt,font=\small] at ({\thE}:\rR) {$E$};
  \node[anchor=north east,inner sep=1.1pt,font=\small] at ({\thF}:\rR) {$F$};
\end{tikzpicture}%
}
\caption{（第14题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: （第3（1）题）曲线(1) ρ=2+2cosθ 与(2) ρ=1/(1−cosθ) 交于 ±45°、±135° 四射线
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\sc{0.52}
  % (1) cardioid ρ=2(1+cosθ) opens right — matches 原图与题内提示
  \draw[line width=0.9pt,domain=0:360,samples=180,smooth]
    plot ({(\sc*(2+2*cos(\x)))*cos(\x)},{(\sc*(2+2*cos(\x)))*sin(\x)});
  % (2) parabola ρ=1/(1-cosθ)
  \draw[line width=0.85pt,domain=28:332,samples=160,smooth]
    plot ({(\sc*(1/(1-cos(\x))))*cos(\x)},{(\sc*(1/(1-cos(\x))))*sin(\x)});
  \foreach \ang in {45,135,-45,-135} {
    \draw[line width=0.55pt] (0,0) -- ({\ang}:{2.85});
  }
  \draw[line width=0.4pt] (0.40,0) arc (0:45:0.40);
  \draw[line width=0.4pt] (0.40,0) arc (0:-45:0.40);
  \draw[line width=0.4pt] (0.55,0) arc (0:135:0.55);
  \draw[line width=0.4pt] (0.55,0) arc (0:-135:0.55);
  \draw[dash pattern=on 1.5pt off 1.05pt,line width=0.5pt] (-0.25,0) -- (0,0);
  \draw[line width=0.65pt,-latex] (0,0) -- (3.35,0) node[anchor=west,inner sep=1.3pt] {$x$};
  \fill (0,0) circle (1.1pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (0,0) {$O$};
  \node[anchor=south,inner sep=1.2pt,font=\small] at (55:{2.6*\sc}) {$(1)$};
  \node[anchor=south west,inner sep=1.0pt,font=\small] at (70:{1.15*\sc}) {$(2)$};
  \node[anchor=south west,inner sep=0.3pt,font=\scriptsize] at (22:0.70) {$45^\circ$};
  \node[anchor=north west,inner sep=0.3pt,font=\scriptsize] at (-22:0.70) {$-45^\circ$};
  \node[anchor=south east,inner sep=0.3pt,font=\scriptsize] at (112:0.82) {$135^\circ$};
  \node[anchor=north east,inner sep=0.3pt,font=\scriptsize] at (-112:0.82) {$-135^\circ$};
\end{tikzpicture}%
}
\caption{（第3（1）题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: （第7题）圆锥曲线 ρ=ep/(1−e cosθ)，焦点 F 为极点；焦点弦 PQ，中点 M(ρ₁,θ)；极轴 Ox
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\ep{1.15}
  \def\ee{1.0}
  \def\th{48}
  \draw[line width=0.9pt,domain=35:325,samples=160,smooth]
    plot ({(\ep/(1-\ee*cos(\x)))*cos(\x)},{(\ep/(1-\ee*cos(\x)))*sin(\x)});
  \pgfmathsetmacro{\rP}{\ep/(1-\ee*cos(\th))}
  \pgfmathsetmacro{\rQ}{\ep/(1-\ee*cos(\th+180))}
  \pgfmathsetmacro{\rM}{(\rP-\rQ)/2}
  \pgfmathsetmacro{\rV}{\ep/(1+\ee)}
  \draw[dash pattern=on 1.5pt off 1.05pt,line width=0.5pt] ({-\rV-0.85},0) -- (0,0);
  \draw[line width=0.65pt,-latex] (0,0) -- (3.2,0) node[anchor=west,inner sep=1.3pt] {$x$};
  \draw[line width=0.75pt] ({\th}:{\rP}) -- ({\th+180}:{\rQ});
  \fill (0,0) circle (1.15pt);
  \fill ({\th}:{\rP}) circle (1.05pt);
  \fill ({\th+180}:{\rQ}) circle (1.05pt);
  \fill ({\th}:{\rM}) circle (1.05pt);
  \node[anchor=north,inner sep=1.4pt,font=\small] at (0,0) {$F$};
  \node[anchor=south west,inner sep=1.2pt,font=\small] at ({\th}:{\rP}) {$P$};
  \node[anchor=north east,inner sep=1.2pt,font=\small] at ({\th+180}:{\rQ}) {$Q$};
  \node[anchor=west,inner sep=1.3pt,font=\small] at ({\th}:{\rM}) {$M(\rho_1,\theta)$};
\end{tikzpicture}%
}
\caption{（第7题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 圆 x²+y²=a² 上点 P′(x′,y′)，同横坐标点 P(x,y) 满足 y:y′=b:a；竖直线过 P′、P 交 x 轴
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\R{2.15}
  \def\bb{1.35}
  \def\th{52}
  \draw[line width=0.55pt,-latex] ({\R+0.35},0) -- ({\R+0.85},0) node[anchor=west,inner sep=1.2pt] {$x$};
  \draw[line width=0.55pt,-latex] (0,{\R+0.25}) -- (0,{\R+0.75}) node[anchor=south,inner sep=1.2pt] {$y$};
  \draw[line width=0.55pt] ({-\R-0.15},0) -- ({\R+0.35},0);
  \draw[line width=0.55pt] (0,{-\R-0.15}) -- (0,{\R+0.25});
  \draw[line width=0.85pt] (0,0) circle ({\R});
  \draw[line width=0.75pt] (0,0) ellipse ({\R} and {\bb});
  \pgfmathsetmacro{\xp}{\R*cos(\th)}
  \pgfmathsetmacro{\yp}{\R*sin(\th)}
  \pgfmathsetmacro{\ye}{\bb/\R*\yp}
  \draw[line width=0.7pt] ({\xp},{\yp}) -- ({\xp},0);
  \fill (0,0) circle (1.1pt);
  \fill ({\xp},{\yp}) circle (1.05pt);
  \fill ({\xp},{\ye}) circle (1.05pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (0,0) {$O$};
  \node[anchor=south west,inner sep=1.3pt,font=\small] at ({\xp},{\yp}) {$P'(x',y')$};
  \node[anchor=west,inner sep=1.4pt,font=\small] at ({\xp},{\ye}) {$P(x,y)$};
\end{tikzpicture}%
}
\caption{（第17（2）题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 圆与同长轴椭圆；上半平面一系列竖矩形顶边在圆上，椭圆横穿各矩形
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\R{2.05}
  \def\bb{1.25}
  \draw[line width=0.5pt,-latex] ({\R+0.3},0) -- ({\R+0.75},0) node[anchor=west,inner sep=1.1pt] {$x$};
  \draw[line width=0.5pt,-latex] (0,{\R+0.2}) -- (0,{\R+0.65}) node[anchor=south,inner sep=1.1pt] {$y$};
  \draw[line width=0.5pt] ({-\R-0.15},0) -- ({\R+0.3},0);
  \draw[line width=0.5pt] (0,{-\R-0.15}) -- (0,{\R+0.2});
  % vertical rectangles under upper semicircle
  \foreach \i in {-9,-8,...,8} {
    \pgfmathsetmacro{\xa}{\R*(\i/9.5)}
    \pgfmathsetmacro{\xb}{\R*((\i+1)/9.5)}
    \pgfmathsetmacro{\xm}{(\xa+\xb)/2}
    \pgfmathsetmacro{\yc}{sqrt(max(\R*\R-\xm*\xm,0.0001))}
    \pgfmathsetmacro{\ye}{\bb/\R*\yc}
    \draw[line width=0.35pt] ({\xa},0) rectangle ({\xb},{\yc});
    \draw[line width=0.35pt] ({\xa},{\ye}) -- ({\xb},{\ye});
  }
  \draw[line width=0.85pt] (0,0) circle ({\R});
  \draw[line width=0.75pt] (0,0) ellipse ({\R} and {\bb});
  \fill (0,0) circle (1.05pt);
...
\centering
\fitfig{%
% fig-alt: 圆 O 直径 AB；弦 MN∥AB，中点 Q；OM 与 AQ 交于 P，∠xOM=θ
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\R{2.05}
  \def\th{38}
  \draw[line width=0.5pt,-latex] ({\R+0.25},0) -- ({\R+0.7},0) node[anchor=west,inner sep=1.1pt] {$x$};
  \draw[line width=0.5pt,-latex] (0,{\R+0.2}) -- (0,{\R+0.6}) node[anchor=south,inner sep=1.1pt] {$y$};
  \draw[line width=0.5pt] ({-\R-0.2},0) -- ({\R+0.25},0);
  \draw[line width=0.5pt] (0,{-\R-0.15}) -- (0,{\R+0.2});
  \draw[line width=0.85pt] (0,0) circle ({\R});
  \pgfmathsetmacro{\ym}{\R*sin(\th)}
  \pgfmathsetmacro{\xm}{\R*cos(\th)}
  % N, M, Q
  \coordinate (O) at (0,0);
  \coordinate (A) at ({\R},0);
  \coordinate (B) at ({-\R},0);
  \coordinate (M) at ({\xm},{\ym});
  \coordinate (N) at ({-\xm},{\ym});
  \coordinate (Q) at (0,{\ym});
  \draw[line width=0.7pt] (N) -- (M);
  \draw[line width=0.7pt] (O) -- (M);
  \draw[line width=0.7pt] (A) -- (Q);
  % P = OM ∩ AQ: OM is line t*(xm,ym); AQ: (R,0)->(0,ym) => (R(1-s), ym*s)
  % R(1-s)=t*xm, ym*s=t*ym => s=t, R-R t = t xm => t=R/(R+xm)
  \pgfmathsetmacro{\tp}{\R/(\R+\xm)}
  \coordinate (P) at ({\tp*\xm},{\tp*\ym});
  \draw[line width=0.4pt] (0.42,0) arc (0:\th:0.42);
  \fill (O) circle (1.05pt);
  \fill (A) circle (1.0pt);
  \fill (B) circle (1.0pt);
  \fill (M) circle (1.0pt);
  \fill (N) circle (1.0pt);
  \fill (Q) circle (1.0pt);
  \fill (P) circle (1.0pt);
  \node[anchor=north east,inner sep=1.1pt,font=\small] at (O) {$O$};
  \node[anchor=north,inner sep=1.3pt,font=\small] at (A) {$A$};
  \node[anchor=north,inner sep=1.3pt,font=\small] at (B) {$B$};
  \node[anchor=south west,inner sep=1.2pt,font=\small] at (M) {$M$};
  \node[anchor=south east,inner sep=1.2pt,font=\small] at (N) {$N$};
  \node[anchor=east,inner sep=1.4pt,font=\small] at (Q) {$Q$};
  \node[anchor=south east,inner sep=1.0pt,font=\small] at (P) {$P$};
  \node[anchor=west,inner sep=0.8pt,font=\small] at ({0.55*cos(\th/2)},{0.55*sin(\th/2)}) {$\theta$};
\end{tikzpicture}%
}
\caption{（第41题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: ∠MON=60°；等边△ABP 边长 a，A 在 OM 上、B 在 ON 上，PA⊥OM
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{1.55}
  \pgfmathsetmacro{\sA}{2*\aa/sqrt(3)}
  \coordinate (O) at (0,0);
  \coordinate (A) at ({\sA},0);
  \coordinate (P) at ({\sA},{\aa});
  \coordinate (B) at ({\aa/(2*sqrt(3))},{\aa/2});
  \draw[line width=0.75pt,-latex] (O) -- (3.35,0) node[anchor=west,inner sep=1.2pt] {$M$};
  \draw[line width=0.75pt,-latex] (O) -- ({3.1*cos(60)},{3.1*sin(60)}) node[anchor=south west,inner sep=1.1pt] {$N$};
  \draw[line width=0.4pt] (0.48,0) arc (0:60:0.48);
  \node[anchor=south west,inner sep=0.6pt,font=\small] at ({0.55*cos(30)},{0.55*sin(30)}) {$60^\circ$};
  \draw[line width=0.85pt] (A) -- (B) -- (P) -- cycle;
  \fill (O) circle (1.05pt);
  \fill (A) circle (1.0pt);
  \fill (B) circle (1.0pt);
  \fill (P) circle (1.0pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (O) {$O$};
  \node[anchor=north,inner sep=1.4pt,font=\small] at (A) {$A$};
  \node[anchor=east,inner sep=1.3pt,font=\small] at (B) {$B$};
  \node[anchor=south,inner sep=1.3pt,font=\small] at (P) {$P$};
  \node[anchor=west,inner sep=1.2pt,font=\small] at ({(\sA+\aa/(2*sqrt(3)))/2},{\aa/4}) {$a$};
  \node[anchor=south east,inner sep=1.0pt,font=\small] at ({(\sA+\aa/(2*sqrt(3)))/2},{({\aa}+\aa/2)/2}) {$a$};
  \node[anchor=west,inner sep=1.5pt,font=\small] at ({\sA},{\aa/2}) {$a$};
\end{tikzpicture}%
}
\caption{（第42题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 极点 O、极轴 Ox；定圆圆心 C(a,0) 半径 r；等边△OAP，A 在圆上，P(ρ,θ)
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\aa{2.55}
  \def\rr{1.15}
  \def\thA{28}
  % A on circle: C + r*(cos phi, sin phi); choose A so OA exists
  \pgfmathsetmacro{\Ax}{\aa+\rr*cos(155)}
  \pgfmathsetmacro{\Ay}{\rr*sin(155)}
  \pgfmathsetmacro{\rhoA}{veclen(\Ax,\Ay)}
  \pgfmathsetmacro{\angA}{atan2(\Ay,\Ax)}
  % P = rotate A around O by +60°
  \pgfmathsetmacro{\angP}{\angA+60}
  \pgfmathsetmacro{\Px}{\rhoA*cos(\angP)}
  \pgfmathsetmacro{\Py}{\rhoA*sin(\angP)}
  \draw[dash pattern=on 1.5pt off 1.05pt,line width=0.5pt] (0,0) -- ({\aa+\rr+0.55},0);
  \draw[line width=0.65pt,-latex] ({\aa+\rr+0.55},0) -- ({\aa+\rr+1.05},0) node[anchor=west,inner sep=1.1pt] {$x$};
  \draw[line width=0.85pt] ({\aa},0) circle ({\rr});
  \draw[line width=0.75pt] (0,0) -- ({\Px},{\Py}) -- ({\Ax},{\Ay}) -- cycle;
  \draw[line width=0.7pt] ({\aa},0) -- ({\Ax},{\Ay});
  \draw[line width=0.4pt] (0.45,0) arc (0:{\angP}:0.45);
  \fill (0,0) circle (1.05pt);
  \fill ({\aa},0) circle (1.0pt);
  \fill ({\Ax},{\Ay}) circle (1.0pt);
  \fill ({\Px},{\Py}) circle (1.0pt);
  \node[anchor=north east,inner sep=1.2pt,font=\small] at (0,0) {$O$};
  \node[anchor=north,inner sep=1.3pt,font=\small] at ({\aa},0) {$C(a,0)$};
  \node[anchor=south east,inner sep=1.1pt,font=\small] at ({\Ax},{\Ay}) {$A$};
  \node[anchor=south west,inner sep=1.2pt,font=\small] at ({\Px},{\Py}) {$P(\rho,\theta)$};
  \node[anchor=south,inner sep=1.0pt,font=\small] at ({0.55*\Px},{0.55*\Py}) {$\rho$};
  \node[anchor=west,inner sep=0.7pt,font=\small] at ({0.55*cos(\angP/2)},{0.55*sin(\angP/2)}) {$\theta$};
  \node[anchor=south west,inner sep=0.8pt,font=\small] at ({(\aa+\Ax)/2},{(\Ay)/2+0.05}) {$r$};
\end{tikzpicture}%
}
\caption{（第44题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 等腰△OAB 顶角 2θ、对称轴 Ox；P 到 OA、OB、AB 的垂足 D、F、E
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\th{28}
  \def\hh{3.1}
  \pgfmathsetmacro{\Ax}{\hh}
  \pgfmathsetmacro{\Ay}{\hh*tan(\th)}
  \coordinate (O) at (0,0);
  \coordinate (A) at ({\Ax},{\Ay});
  \coordinate (B) at ({\Ax},{-\Ay});
  \coordinate (P) at (1.55,0.42);
  % feet: D = proj of P on OA; F on OB; E on AB (x=Ax)
  % OA direction (cos th, sin th) in terms of angle of OA = th
  \pgfmathsetmacro{\pOA}{1.55*cos(\th)+0.42*sin(\th)}
  \coordinate (D) at ({\pOA*cos(\th)},{\pOA*sin(\th)});
  \pgfmathsetmacro{\pOB}{1.55*cos(-\th)+0.42*sin(-\th)}
  \coordinate (F) at ({\pOB*cos(-\th)},{\pOB*sin(-\th)});
  \coordinate (E) at ({\Ax},0.42);
  \draw[line width=0.5pt,-latex] ({\Ax+0.35},0) -- ({\Ax+0.85},0) node[anchor=west,inner sep=1.1pt] {$x$};
  \draw[line width=0.5pt,-latex] (0,{\Ay+0.35}) -- (0,{\Ay+0.75}) node[anchor=south,inner sep=1.1pt] {$y$};
  \draw[line width=0.5pt] (-0.25,0) -- ({\Ax+0.35},0);
  \draw[line width=0.5pt] (0,{-\Ay-0.25}) -- (0,{\Ay+0.35});
  \draw[line width=0.85pt] (O) -- (A) -- (B) -- cycle;
  \draw[line width=0.7pt] (P) -- (D);
  \draw[line width=0.7pt] (P) -- (F);
  \draw[dash pattern=on 1.4pt off 1.0pt,line width=0.55pt] (P) -- (E);
  \draw[line width=0.35pt] (0.55,0) arc (0:\th:0.55);
  \draw[line width=0.35pt] (0.55,0) arc (0:{-\th}:0.55);
  \fill (O) circle (1.05pt);
  \fill (A) circle (1.0pt);
  \fill (B) circle (1.0pt);
  ...
  \fill (E) circle (0.9pt);
  \node[anchor=north east,inner sep=1.1pt,font=\small] at (O) {$O$};
  \node[anchor=south west,inner sep=1.2pt,font=\small] at (A) {$A$};
  \node[anchor=north west,inner sep=1.2pt,font=\small] at (B) {$B$};
  \node[anchor=south,inner sep=1.3pt,font=\small] at (P) {$P$};
  \node[anchor=south east,inner sep=1.0pt,font=\small] at (D) {$D$};
  \node[anchor=north east,inner sep=1.0pt,font=\small] at (F) {$F$};
  \node[anchor=west,inner sep=1.3pt,font=\small] at (E) {$E$};
  \node[anchor=south west,inner sep=0.5pt,font=\small] at ({0.7*cos(\th/2)},{0.7*sin(\th/2)}) {$\theta$};
  \node[anchor=north west,inner sep=0.5pt,font=\small] at ({0.7*cos(-\th/2)},{0.7*sin(-\th/2)}) {$\theta$};
\end{tikzpicture}%
}
\caption{（第4题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 正方形 ABCD 内侧等边△ABK、BCL、CDM、DAN；KL/LM/MN/NK 与诸边中点构成正十二边形 P₁…P₁₂
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  \def\s{3.4}
  \pgfmathsetmacro{\h}{\s*sqrt(3)/2}
  \coordinate (A) at (0,0);
  \coordinate (B) at ({\s},0);
  \coordinate (C) at ({\s},{\s});
  \coordinate (D) at (0,{\s});
  % inward equilateral apexes (K on AB, L on BC, M on CD, N on DA)
  \coordinate (K) at ({\s/2},{\h});
  \coordinate (L) at ({\s-\h},{\s/2});
  \coordinate (M) at ({\s/2},{\s-\h});
  \coordinate (N) at ({\h},{\s/2});
  \draw[line width=0.9pt] (A) -- (B) -- (C) -- (D) -- cycle;
  \draw[line width=0.7pt] (A) -- (K) -- (B);
  \draw[line width=0.7pt] (B) -- (L) -- (C);
  \draw[line width=0.7pt] (C) -- (M) -- (D);
  \draw[line width=0.7pt] (D) -- (N) -- (A);
  \draw[line width=0.7pt] (K) -- (L) -- (M) -- (N) -- cycle;
  % 12 midpoints of regular dodecagon claim
  \coordinate (PAK) at ($(A)!0.5!(K)$);
  \coordinate (PBK) at ($(B)!0.5!(K)$);
  \coordinate (PKL) at ($(K)!0.5!(L)$);
  \coordinate (PBL) at ($(B)!0.5!(L)$);
  \coordinate (PCL) at ($(C)!0.5!(L)$);
  \coordinate (PLM) at ($(L)!0.5!(M)$);
  \coordinate (PCM) at ($(C)!0.5!(M)$);
  \coordinate (PDM) at ($(D)!0.5!(M)$);
  \coordinate (PMN) at ($(M)!0.5!(N)$);
  \coordinate (PDN) at ($(D)!0.5!(N)$);
  \coordinate (PAN) at ($(A)!0.5!(N)$);
  \coordinate (PNK) at ($(N)!0.5!(K)$);
  \foreach \pt in {PAK,PBK,PKL,PBL,PCL,PLM,PCM,PDM,PMN,PDN,PAN,PNK}
    \fill (\pt) circle (0.85pt);
  \fill (A) circle (1.0pt);
  \fill (B) circle (1.0pt);
  \fill (C) circle (1.0pt);
  \fill (D) circle (1.0pt);
  ...
  \fill (M) circle (1.0pt);
  \fill (N) circle (1.0pt);
  \node[anchor=north east,inner sep=1.1pt,font=\small] at (A) {$A$};
  \node[anchor=north west,inner sep=1.1pt,font=\small] at (B) {$B$};
  \node[anchor=south west,inner sep=1.1pt,font=\small] at (C) {$C$};
  \node[anchor=south east,inner sep=1.1pt,font=\small] at (D) {$D$};
  \node[anchor=south,inner sep=1.2pt,font=\small] at (K) {$K$};
  \node[anchor=west,inner sep=1.2pt,font=\small] at (L) {$L$};
  \node[anchor=north,inner sep=1.2pt,font=\small] at (M) {$M$};
  \node[anchor=east,inner sep=1.2pt,font=\small] at (N) {$N$};
  \node[anchor=east,inner sep=0.6pt,font=\scriptsize] at (PNK) {$P_{1}$};
  \node[anchor=west,inner sep=0.6pt,font=\scriptsize] at (PKL) {$P_{2}$};
  \node[anchor=south,inner sep=0.5pt,font=\scriptsize] at (PBL) {$P_{3}$};
  \node[anchor=south,inner sep=0.5pt,font=\scriptsize] at (PCL) {$P_{4}$};
  \node[anchor=east,inner sep=0.5pt,font=\scriptsize] at (PLM) {$P_{5}$};
  \node[anchor=west,inner sep=0.5pt,font=\scriptsize] at (PCM) {$P_{6}$};
  \node[anchor=west,inner sep=0.5pt,font=\scriptsize] at (PDM) {$P_{7}$};
  \node[anchor=east,inner sep=0.5pt,font=\scriptsize] at (PMN) {$P_{8}$};
  \node[anchor=north,inner sep=0.5pt,font=\scriptsize] at (PDN) {$P_{9}$};
  \node[anchor=north,inner sep=0.5pt,font=\scriptsize] at (PAN) {$P_{10}$};
  \node[anchor=west,inner sep=0.5pt,font=\scriptsize] at (PAK) {$P_{11}$};
  \node[anchor=east,inner sep=0.5pt,font=\scriptsize] at (PBK) {$P_{12}$};
\end{tikzpicture}%
}
\caption{（第5题）}
\end{figure}
...
\centering
\fitfig{%
% fig-alt: 抛物线 y²=2x；弦 P₁P₂ 中点 M₁ 作轴平行线交曲线于 P₃；再于 P₁P₃、P₃P₂ 上取 M₂、M₃ 得 Q₁、Q₂
\begin{tikzpicture}[x=1cm,y=1cm,line cap=round,line join=round,>=latex]
  % y^2 = 2 x  => x = y^2/2
  \def\yone{2.35}
  \def\ytwo{-1.55}
  \pgfmathsetmacro{\xone}{\yone*\yone/2}
  \pgfmathsetmacro{\xtwo}{\ytwo*\ytwo/2}
  \pgfmathsetmacro{\ythree}{(\yone+\ytwo)/2}
  \pgfmathsetmacro{\xthree}{\ythree*\ythree/2}
  \pgfmathsetmacro{\xmone}{(\xone+\xtwo)/2}
  \pgfmathsetmacro{\ymone}{\ythree}
  \pgfmathsetmacro{\ytqone}{(\yone+\ythree)/2}
  \pgfmathsetmacro{\xtqone}{\ytqone*\ytqone/2}
  \pgfmathsetmacro{\xmtwo}{(\xone+\xthree)/2}
  \pgfmathsetmacro{\ymtwo}{\ytqone}
  \pgfmathsetmacro{\ytqtwo}{(\ythree+\ytwo)/2}
  \pgfmathsetmacro{\xtqtwo}{\ytqtwo*\ytqtwo/2}
  \pgfmathsetmacro{\xmthree}{(\xthree+\xtwo)/2}
  \pgfmathsetmacro{\ymthree}{\ytqtwo}
  \pgfmathsetmacro{\xmax}{\xone+0.55}
  \draw[line width=0.5pt,-latex] ({\xmax},0) -- ({\xmax+0.45},0) node[anchor=west,inner sep=1.1pt] {$x$};
  \draw[line width=0.5pt,-latex] (0,2.7) -- (0,3.15) node[anchor=south,inner sep=1.1pt] {$y$};
  \draw[line width=0.5pt] (-0.35,0) -- ({\xmax},0);
  \draw[line width=0.5pt] (0,-2.0) -- (0,2.7);
  \draw[line width=0.9pt,domain=-2.55:2.55,samples=80,smooth]
    plot ({\x*\x/2},{\x});
  \coordinate (P1) at ({\xone},{\yone});
  \coordinate (P2) at ({\xtwo},{\ytwo});
  \coordinate (P3) at ({\xthree},{\ythree});
  \coordinate (M1) at ({\xmone},{\ymone});
  \coordinate (M2) at ({\xmtwo},{\ymtwo});
  \coordinate (M3) at ({\xmthree},{\ymthree});
  \coordinate (Q1) at ({\xtqone},{\ytqone});
  \coordinate (Q2) at ({\xtqtwo},{\ytqtwo});
  \draw[line width=0.75pt] (P1) -- (P2) -- (P3) -- cycle;
  \draw[line width=0.65pt] (P3) -- (M1);
  \draw[line width=0.65pt] (Q1) -- (M2);
  \draw[line width=0.65pt] (Q2) -- (M3);
  \draw[line width=0.65pt] (P1) -- (Q1) -- (P3);
  \draw[line width=0.65pt] (P3) -- (Q2) -- (P2);
  \fill (0,0) circle (1.05pt);
  \foreach \pt in {P1,P2,P3,M1,M2,M3,Q1,Q2} \fill (\pt) circle (1.0pt);
  \node[anchor=north east,inner sep=1.1pt,font=\small] at (0,0) {$O$};
  \node[anchor=south west,inner sep=1.1pt,font=\small] at (P1) {$P_1$};
  \node[anchor=north west,inner sep=1.1pt,font=\small] at (P2) {$P_2$};
  \node[anchor=east,inner sep=1.2pt,font=\small] at (P3) {$P_3$};
  \node[anchor=west,inner sep=1.2pt,font=\small] at (M1) {$M_1$};
  \node[anchor=south,inner sep=1.0pt,font=\small] at (M2) {$M_2$};
  \node[anchor=north,inner sep=1.0pt,font=\small] at (M3) {$M_3$};
  \node[anchor=east,inner sep=1.1pt,font=\small] at (Q1) {$Q_1$};
  \node[anchor=east,inner sep=1.1pt,font=\small] at (Q2) {$Q_2$};
\end{tikzpicture}%
}
\caption{（第9题）}
\end{figure}
