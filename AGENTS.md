...
- 提交前运行 `make audit` 和 `make tex-audit`；推送前 `make pre-push` 必须通过。构建 PDF 用 `make pdf`（GNU Make + XeLaTeX，不经 Python/Pandoc）。单册：`make dist/代数（第一册）.pdf`。合订本源是 `tex/collections/` 下三份合订 `.tex`，由 XeLaTeX 直接编译，页码连续；不要用 Ghostscript 钉分册 PDF。
- `dist/`、`.build/`、`reports/` 和 `.agents/` 属于生成或本地状态，不纳入版本控制。`tex/vendor/fonts/*.otf` 按 Git binary 处理，不要当文本转换换行。
